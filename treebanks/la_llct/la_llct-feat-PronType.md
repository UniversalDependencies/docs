...
20065 <tt><a href="la_llct-pos-DET.html">DET</a></tt> tokens (100% of all `DET` tokens) have a non-empty value of `PronType`.

The most frequent other feature values with which `DET` and `PronType` co-occurred: <tt><a href="la_llct-feat-Number-psor.html">Number[psor]</a></tt><tt>=EMPTY</tt> (14838; 74%), <tt><a href="la_llct-feat-Number.html">Number</a></tt><tt>=Sing</tt> (14697; 73%), <tt><a href="la_llct-feat-Person-psor.html">Person[psor]</a></tt><tt>=EMPTY</tt> (14136; 70%), <tt><a href="la_llct-feat-Poss.html">Poss</a></tt><tt>=EMPTY</tt> (14136; 70%).

`DET` tokens may have the following values of `PronType`:

* `Con` (748; 4% of non-empty `PronType`): <em>alio, alia, aliis, alie, alii, alios, aliam, aliud, utraque, alium</em>
* `Dem` (8832; 44% of non-empty `PronType`): <em>ipsa, suprascripta, ipsius, hanc, ipse, illa, ipso, suprascripte, suprascripto, hec</em>
* `Ind` (2401; 12% of non-empty `PronType`): <em>qui, uno, una, quolibet, unum, unam, cuius, qualibet, aliquantis, quas</em>
* `Prs` (5929; 30% of non-empty `PronType`): <em>mea, nostro, tuis, meis, vestro, suis, meo, suas, nostri, vestre</em>
* `Rel` (247; 1% of non-empty `PronType`): <em>quales, quot, quale, quantas, qualis, quanta, quali, quante, quiscunque, qualem</em>
* `Tot` (1908; 10% of non-empty `PronType`): <em>omnia, omnibus, omni, omnem, omnes, ambas, omne, omnis, universis, ambo</em>
...
`PronType` seems to be **lexical feature** of `DET`. 98% lemmas (41) occur only with one value of `PronType`.
...
* `Con` (3; 0% of non-empty `PronType`): <em>aliter</em>
* `Dem` (1245; 50% of non-empty `PronType`): <em>taliter, tam, tantum, hic, ita, tantu, hinc, ic</em>
* `Ind` (7; 0% of non-empty `PronType`): <em>nihil, aliquantum</em>
* `Neg` (6; 0% of non-empty `PronType`): <em>numquam, numqua, nunquam</em>
* `Rel` (1249; 50% of non-empty `PronType`): <em>ibi, ubi, quantum, unde, ubicumque, qualiter, ut, quamvis, quandoque, quantu</em>
* `EMPTY` (5656): <em>supra, una, exinde, sic, superius, tunc, iam, vero, bene, modo</em>

`PronType` seems to be **lexical feature** of `ADV`. 100% lemmas (23) occur only with one value of `PronType`.

### `SCONJ`

1031 <tt><a href="la_llct-pos-SCONJ.html">SCONJ</a></tt> tokens (24% of all `SCONJ` tokens) have a non-empty value of `PronType`.

`SCONJ` tokens may have the following values of `PronType`:

* `Rel` (1031; 100% of non-empty `PronType`): <em>ut, quam, quando, ud, cum, quandoque, u, aut, cu, qualiter</em>
* `EMPTY` (3268): <em>si, quia, sicut, qualiter, dum, postquam, quod, quam, quomodo, quatenus</em>

### `PART`

18 <tt><a href="la_llct-pos-PART.html">PART</a></tt> tokens (2% of all `PART` tokens) have a non-empty value of `PronType`.

The most frequent other feature values with which `PART` and `PronType` co-occurred: <tt><a href="la_llct-feat-Polarity.html">Polarity</a></tt><tt>=EMPTY</tt> (18; 100%).

`PART` tokens may have the following values of `PronType`:

* `Dem` (18; 100% of non-empty `PronType`): <em>ecce, hecce</em>
* `EMPTY` (858): <em>non, nam, ne, quidem, autem, enim, igitur, namque, itaque</em>

### `CCONJ`

12 <tt><a href="la_llct-pos-CCONJ.html">CCONJ</a></tt> tokens (0% of all `CCONJ` tokens) have a non-empty value of `PronType`.

`CCONJ` tokens may have the following values of `PronType`:

* `Dem` (1; 8% of non-empty `PronType`): <em>tam</em>
* `Rel` (11; 92% of non-empty `PronType`): <em>ut, ud</em>
* `EMPTY` (13981): <em>et, vel, aut, seo, adque, seu, nisi, que, sive, ac</em>
...
<tt>ADV --[<tt><a href="la_llct-dep-fixed.html">fixed</a></tt>]--> SCONJ</tt> (1; 100%),
