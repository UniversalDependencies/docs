...
1138 tokens (1%) have a non-empty value of `Case`.
86 types (1%) occur at least once with a non-empty value of `Case`.
83 lemmas (1%) occur at least once with a non-empty value of `Case`.
The feature is used with 3 part-of-speech tags: <tt><a href="fa_seraji-pos-ADV.html">ADV</a></tt> (1108; 1% instances), <tt><a href="fa_seraji-pos-INTJ.html">INTJ</a></tt> (26; 0% instances), <tt><a href="fa_seraji-pos-NOUN.html">NOUN</a></tt> (4; 0% instances).

### `ADV`

1108 <tt><a href="fa_seraji-pos-ADV.html">ADV</a></tt> tokens (29% of all `ADV` tokens) have a non-empty value of `Case`.
...
<table>
  ...
  <tr><td><tt></tt></td><td>پیش</td><td>پیش</td></tr>
</table>

`Case` seems to be **lexical feature** of `ADV`. 99% lemmas (78) occur only with one value of `Case`.

### `INTJ`

26 <tt><a href="fa_seraji-pos-INTJ.html">INTJ</a></tt> tokens (38% of all `INTJ` tokens) have a non-empty value of `Case`.

`INTJ` tokens may have the following values of `Case`:

* `Voc` (26; 100% of non-empty `Case`): ای
* `EMPTY` (43): بله، خیر، آره، آری، ا، هی، نه، وای، آ، آخ
