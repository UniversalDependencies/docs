...
However, due to Mandarin and standard written Chinese influence, the Mandarin 被 / _bèi_ (_bei6_ in Cantonese) is sometimes used in formal registers such as newscast, in which case if it is followed by an agent noun, it is treated the same way as the Cantonese 畀 / _bei2_ here, as a prepositional noun marker. Otherwise, if it is used singly without an agent noun, it is treated as a passive auxiliary (see [AUX]() and [aux]().
