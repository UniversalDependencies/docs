...
[have been properly documented](/contributing/language_specific.html). If a feature or relation does not have
...
new ones. (As soon as you edit the first undocumented auxiliary, all other undocumented auxiliaries
will be removed from the database to make room for the new and proper definitions. You should thus
not stop after the first auxiliary. You should document all the others so that no existing treebanks
...
in cases of code-switching, i.e., when a [foreign language phrase is inserted](/foreign.html) in the text, and the
...
**WARNING:** `validate.py` is only one of the scripts that are used to verify whether your treebank
is good for the next release. Even if you run it on your computer and it tells you “PASSED”, you must
check the [online validation report](http://quest.ms.mff.cuni.cz/udvalidator/) after you push changes
to GitHub. Only if your treebank is reported there as “VALID”, you are all set.
