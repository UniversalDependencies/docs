...
* Words are generally delimited by whitespace but with exceptions:
  * Compound words and reduplications that are separated by a hyphen are tokenized as multi-word tokens (e.g. adis + - + adis) if each word can be taken as a separate semantic expression forming a whole.
