...
* `Art` (3928; 74% of non-empty `PronType`): <em>the, a, an, another, Le, les, une</em>
...
`PronType` seems to be **lexical feature** of `DET`. 91% lemmas (31) occur only with one value of `PronType`.

### `PRON`

2042 <tt><a href="en_partut-pos-PRON.html">PRON</a></tt> tokens (100% of all `PRON` tokens) have a non-empty value of `PronType`.

The most frequent other feature values with which `PRON` and `PronType` co-occurred: <tt><a href="en_partut-feat-Gender.html">Gender</a></tt><tt>=EMPTY</tt> (1819; 89%).

`PRON` tokens may have the following values of `PronType`:

* `Dem` (224; 11% of non-empty `PronType`): <em>there, this, that, those, these</em>
* `Emp` (2; 0% of non-empty `PronType`): <em>himself, themselves</em>
* `Ind` (117; 6% of non-empty `PronType`): <em>all, some, others, each, nothing, Many, one, other, Much, both</em>
* `Int` (18; 1% of non-empty `PronType`): <em>what, who</em>
* `Prs` (1263; 62% of non-empty `PronType`): <em>it, I, we, he, you, they, them, him, everyone, us</em>
* `Rcp` (3; 0% of non-empty `PronType`): <em>one</em>
* `Rel` (415; 20% of non-empty `PronType`): <em>which, that, who, what, whom, whose, when, whereby</em>
...
* `EMPTY` (1814): <em>more, also, now, only, as, so, how, however, well, very</em>
