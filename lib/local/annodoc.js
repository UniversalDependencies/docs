...
//         		      function(a, b) { return b.replace(/\s+/g, ' '); });
    ...
    var ConllUSentence = function(elements, comments) {
    ...
    var parseConllU = function(conll, log, strict) {
        ...
        for (var idx=0; idx<lines.length; idx++) {
            ...
            if (line.length !== 0 && line[0] === '#') {
                // comment line, check and store
                if (!beforeSentence) {
                    log('error: comment lines must precede a sentence');
                    error = true;
                } else {
                    comments.push(line);
                }
            ...
            if (fields.length === 0) {
                ...
                if (elements.length === 0) {
                    ...
                    sentences.push(new ConllUSentence(elements, comments));
                }
                // reset
                elements = [];
                comments = [];
        ...
        if (comments.length !== 0) {
            log('error: comments may not occur after last sentence');
            error = true;
        }
