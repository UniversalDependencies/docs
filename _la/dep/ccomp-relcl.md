...
This syntactic subrelation (also implemented for [csubj](la-dep/ccomp-relcl) and [advcl](la-dep/advcl-relcl)) is used to distinguish a so-called free [relative clause](la-dep/acl-relcl) from other occurrences of [clauses acting as arguments](la-dep/ccomp) in another matrix clause, according to its function. 
...
* *quod* is the neuter singular nominative/accusative form of *qui* 'that, who, which', which is annotated with `Case=Acc`, as it is the object of *inferre* 'inferring'. We notice that the relative pronoun appears embedded two levels below the head of the free relative clause: Latin has no restrictions whatsoever to what can be relativised. 
