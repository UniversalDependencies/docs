...
* `Yes` (55; 100% of non-empty `Typo`): <em>сдесь, вобще, както, переодически, Меног, Одеажды, Собстна, безконечно, больши, братский</em>

`Typo` seems to be **lexical feature** of `ADV`. 100% lemmas (41) occur only with one value of `Typo`.
...
16 <tt><a href="ru_taiga-pos-ADP.html">ADP</a></tt> tokens (0% of all `ADP` tokens) have a non-empty value of `Typo`.

`ADP` tokens may have the following values of `Typo`:

* `Yes` (16; 100% of non-empty `Typo`): <em>а, и, в, К, Ут, восле, заа, иза, п, с</em>

### `PRON`

14 <tt><a href="ru_taiga-pos-PRON.html">PRON</a></tt> tokens (0% of all `PRON` tokens) have a non-empty value of `Typo`.

The most frequent other feature values with which `PRON` and `Typo` co-occurred: <tt><a href="ru_taiga-feat-Number.html">Number</a></tt><tt>=Sing</tt> (12; 86%), <tt><a href="ru_taiga-feat-Person.html">Person</a></tt><tt>=EMPTY</tt> (10; 71%), <tt><a href="ru_taiga-feat-Case.html">Case</a></tt><tt>=Nom</tt> (8; 57%).

`PRON` tokens may have the following values of `Typo`:

* `Yes` (14; 100% of non-empty `Typo`): <em>сто, Нечего, Тоже, клторые, которые, некому, нё, оеа, она, от</em>

### `PART`

10 <tt><a href="ru_taiga-pos-PART.html">PART</a></tt> tokens (0% of all `PART` tokens) have a non-empty value of `Typo`.

The most frequent other feature values with which `PART` and `Typo` co-occurred: <tt><a href="ru_taiga-feat-Polarity.html">Polarity</a></tt><tt>=EMPTY</tt> (6; 60%).
...
4 <tt><a href="ru_taiga-pos-CCONJ.html">CCONJ</a></tt> tokens (0% of all `CCONJ` tokens) have a non-empty value of `Typo`.

`CCONJ` tokens may have the following values of `Typo`:

* `Yes` (4; 100% of non-empty `Typo`): <em>Иди, ин, лбо, нр</em>
...
* `Yes` (3; 100% of non-empty `Typo`): <em>з, оден, скольки</em>

### `SCONJ`

3 <tt><a href="ru_taiga-pos-SCONJ.html">SCONJ</a></tt> tokens (0% of all `SCONJ` tokens) have a non-empty value of `Typo`.

`SCONJ` tokens may have the following values of `Typo`:

* `Yes` (3; 100% of non-empty `Typo`): <em>чтл, я, Чтоп</em>

### `INTJ`

2 <tt><a href="ru_taiga-pos-INTJ.html">INTJ</a></tt> tokens (1% of all `INTJ` tokens) have a non-empty value of `Typo`.

`INTJ` tokens may have the following values of `Typo`:

* `Yes` (2; 100% of non-empty `Typo`): <em>бл@, бляь</em>
...
<tt>ADV --[<tt><a href="ru_taiga-dep-goeswith.html">goeswith</a></tt>]--> ADV</tt> (1; 100%),
<tt>NOUN --[<tt><a href="ru_taiga-dep-conj.html">conj</a></tt>]--> DET</tt> (1; 100%),
<tt>NOUN --[<tt><a href="ru_taiga-dep-nmod.html">nmod</a></tt>]--> ADJ</tt> (1; 100%),
<tt>VERB --[<tt><a href="ru_taiga-dep-advcl.html">advcl</a></tt>]--> ADV</tt> (1; 100%),
<tt>VERB --[<tt><a href="ru_taiga-dep-obl-agent.html">obl:agent</a></tt>]--> PROPN</tt> (1; 100%).
