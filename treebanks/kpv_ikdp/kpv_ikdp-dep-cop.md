---
layout: base
title:  'Statistics of cop in UD_Komi_Zyrian-IKDP'
udver: '2'
---

## Treebank Statistics: UD_Komi_Zyrian-IKDP: Relations: `cop`

This relation is universal.

21 nodes (1%) are attached to their parents as `cop`.

15 instances of `cop` (71%) are left-to-right (parent precedes child).
Average distance between parent and child is 1.9047619047619.

The following 4 pairs of parts of speech are connected with `cop`: <tt><a href="kpv_ikdp-pos-NOUN.html">NOUN</a></tt>-<tt><a href="kpv_ikdp-pos-AUX.html">AUX</a></tt> (16; 76% instances), <tt><a href="kpv_ikdp-pos-ADV.html">ADV</a></tt>-<tt><a href="kpv_ikdp-pos-AUX.html">AUX</a></tt> (2; 10% instances), <tt><a href="kpv_ikdp-pos-PRON.html">PRON</a></tt>-<tt><a href="kpv_ikdp-pos-AUX.html">AUX</a></tt> (2; 10% instances), <tt><a href="kpv_ikdp-pos-ADJ.html">ADJ</a></tt>-<tt><a href="kpv_ikdp-pos-AUX.html">AUX</a></tt> (1; 5% instances).


~~~ conllu
# visual-style 2	bgColor:blue
# visual-style 2	fgColor:white
# visual-style 5	bgColor:blue
# visual-style 5	fgColor:white
# visual-style 5 2 cop	color:blue
1	Миян	ми	PRON	Pron	Case=Gen|Number=Plur|Person=1|PronType=Prs	5	nmod	_	_
2	выйым	эм	AUX	V	Number=Sing	5	cop	_	_
3	асланум	ас	PRON	Pron	Case=Gen|Person=1|PronType=Prs|Reflex=Yes	5	nmod	_	SpaceAfter=No
4	,	,	PUNCT	CLB	_	5	punct	_	_
5	керка	керка	NOUN	N	Case=Nom|Number=Sing	0	root	_	SpaceAfter=No
6	,	,	PUNCT	CLB	_	7	punct	_	_
7	мӧс	мӧс	NOUN	N	Case=Nom|Number=Sing	5	list	_	SpaceAfter=No
8	,	,	PUNCT	CLB	_	9	punct	_	_
9	ыж	ыж	NOUN	N	Case=Nom|Number=Sing	5	list	_	SpaceAfter=No
10	,	,	PUNCT	CLB	_	11	punct	_	_
11	кукань	кукань	NOUN	N	Case=Nom|Number=Sing	5	list	_	SpaceAfter=No
12	.	.	PUNCT	CLB	_	5	punct	_	_

~~~


~~~ conllu
# visual-style 24	bgColor:blue
# visual-style 24	fgColor:white
# visual-style 22	bgColor:blue
# visual-style 22	fgColor:white
# visual-style 22 24 cop	color:blue
1	Ну	ну	PART	Pcle	_	3	cc	_	OrigLang=ru|SpaceAfter=No|Note=CheckAll
2	,	,	PUNCT	CLB	_	3	punct	_	_
3	палялі	палявны	VERB	V	Mood=Ind|Number=Sing|Person=1|Tense=Past|VerbForm=Fin	0	root	_	_
4	ас	ас	ADJ	A	Case=Nom|Number=Sing|PronType=Prs	5	amod	_	_
5	местэам	местӧ	NOUN	N	Case=Ill|Number=Sing|Number[psor]=Sing|Person[psor]=1	3	obl	_	OrigLang=ru|SpaceAfter=No
6	,	,	PUNCT	CLB	_	7	punct	_	_
7	а	а	CCONJ	CC	_	9	cc	_	OrigLang=ru
8	мӧд	мӧд	PRON	Pron	PronType=Dem	9	det	_	_
9	луннас	лун	NOUN	N	Case=Ins|Number=Sing|Number[psor]=Sing|Person[psor]=3	3	acl	_	_
10	ми	ми	PRON	Pron	Case=Nom|Number=Plur|Person=1|PronType=Prs	15	nsubj	_	_
11	уже	уже	ADV	Adv	_	15	advmod	_	OrigLang=ru|SpaceAfter=No
12	,	,	PUNCT	CLB	_	11	punct	_	_
13	абу	абу	AUX	Pcle	Polarity=Neg	15	aux:neg	_	_
14	мӧд	мӧд	PRON	Pron	PronType=Dem	15	det	_	_
15	луннас-а	луннас	NOUN	N	Case=Ins|Number=Sing|Number[psor]=Sing|Person[psor]=3	22	csubj	_	SpaceAfter=No
16	,	,	PUNCT	CLB	_	15	punct	_	_
17	сы	сійӧ	PRON	Pron	Case=Nom|Number=Sing|Person=3|PronType=Prs	18	det	_	_
18	луннас	луннас	NOUN	N	Case=Ins|Number=Sing|Number[psor]=Sing|Person[psor]=3	15	conj	_	_
19	же	жӧ	PART	Pcle	_	18	advmod	_	OrigLang=ru
20	вот	вот	PART	Pcle	_	22	advmod	_	OrigLang=ru|SpaceAfter=No
21	,	,	PUNCT	CLB	_	20	punct	_	_
22	тытэн	тытэн	ADV	Adv	_	3	advcl	_	_
23	нин	нин	ADV	Adv	_	22	advmod	_	_
24	вӧлі	вӧвны	AUX	V	Mood=Ind|Number=Sing|Person=3|Tense=Past|VerbForm=Fin	22	cop	_	SpaceAfter=No
25	.	.	PUNCT	CLB	_	3	punct	_	_

~~~


~~~ conllu
# visual-style 3	bgColor:blue
# visual-style 3	fgColor:white
# visual-style 2	bgColor:blue
# visual-style 2	fgColor:white
# visual-style 2 3 cop	color:blue
1	А	а	CCONJ	CC	_	2	cc	_	_
2	нылэн	сійӧ	PRON	Pron	Case=Gen|Number=Plur|Person=3|PronType=Prs	6	reparandum	_	GTtags=Pers,Pl3,Gen,Err/Dial
3	вӧлі	вӧвны	AUX	_	Mood=Ind|Number=Sing|Person=3|Tense=Past|VerbForm=Fin	2	cop	_	GTtags=Ind,Prt1,Sg3|SpaceAfter=No
4	,	,	PUNCT	PUNCT	_	6	punct	_	_
5	нёль	нёль	NUM	Num	Case=Nom|Number=Sing|NumType=Card	6	nummod	_	GTtags=Card,Sg,Nom
6	воклэн	вок	NOUN	N	Case=Gen|Number=Sing	0	root	_	GTtags=Sg,Err/Dial,Gen
7	ӧтік	ӧти	NUM	Num	Case=Nom|Number=Sing|NumType=Card	10	nummod	_	GTtags=Err/Dial,Card,Sg,Nom
8	небыд	небыд	ADJ	A	Case=Nom|Number=Sing	9	amod	_	GTtags=Sg,Nom
9	юрси	юрси	NOUN	N	Case=Nom|Number=Sing	10	nmod	_	GTtags=Sg,Nom
10	чой	чой	NOUN	N	Case=Nom|Number=Sing	6	nsubj:cop	_	GTtags=Sg,Nom|SpaceAfter=No
11	,	,	PUNCT	PUNCT	_	14	punct	_	_
12	небыд	небыд	ADJ	A	Case=Nom|Number=Sing	13	amod	_	GTtags=Sg,Nom
13	юрси	юрси	NOUN	N	Case=Nom|Number=Sing	14	nmod	_	GTtags=Sg,Nom
14	чойныс	чой	NOUN	N	Case=Nom|Number=Sing|Number[psor]=Plur|Person[psor]=3	10	conj	_	GTtags=Sg,Nom,PxPl3|SpaceAfter=No
15	.	.	PUNCT	PUNCT	_	6	punct	_	_

~~~


