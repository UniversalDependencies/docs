

--------------------------------------------------------------------------------

## Treebank Statistics (UD_English)

This feature is universal.
It occurs with 2 different values: `Imp`, `Ind`.

20676 tokens (8%) have a non-empty value of `Mood`.
1992 types (10%) occur at least once with a non-empty value of `Mood`.
1084 lemmas (7%) occur at least once with a non-empty value of `Mood`.
The feature is used with 3 part-of-speech tags: [en-pos/VERB]() (15990; 6% instances), [en-pos/AUX]() (4685; 2% instances), [en-pos/ADP]() (1; 0% instances).

### `VERB`

15990 [en-pos/VERB]() tokens (47% of all `VERB` tokens) have a non-empty value of `Mood`.

The most frequent other feature values with which `VERB` and `Mood` co-occurred: <tt><a href="VerbForm.html">VerbForm</a>=Fin</tt> (15990; 100%), <tt><a href="Person.html">Person</a>=EMPTY</tt> (10310; 64%), <tt><a href="Number.html">Number</a>=EMPTY</tt> (10299; 64%), <tt><a href="Tense.html">Tense</a>=Pres</tt> (9248; 58%).

`VERB` tokens may have the following values of `Mood`:

* `Imp` (1466; 9% of non-empty `Mood`): <em>let, go, see, take, try, get, make, be, give, call</em>
* `Ind` (14524; 91% of non-empty `Mood`): <em>is, was, are, have, 's, had, were, said, has, 'm</em>
* `EMPTY` (18088): <em>be, have, get, know, do, go, going, been, make, take</em>

<table>
  <tr><th>Paradigm <i>be</i></th><th><tt>Ind</tt></th><th><tt>Imp</tt></th></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=1|<a href="Tense.html">Tense</a>=Past</tt></td><td><em>was</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=1|<a href="Tense.html">Tense</a>=Pres</tt></td><td><em>am</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=3|<a href="Tense.html">Tense</a>=Past</tt></td><td><em>was</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=3|<a href="Tense.html">Tense</a>=Pres</tt></td><td><em>is, 's, s, ai</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Tense.html">Tense</a>=Past</tt></td><td><em>was</em></td><td></td></tr>
  <tr><td><tt><a href="Tense.html">Tense</a>=Past</tt></td><td><em>were</em></td><td></td></tr>
  <tr><td><tt><a href="Tense.html">Tense</a>=Pres</tt></td><td><em>are, 'm, 're, m, r, re, is</em></td><td></td></tr>
  <tr><td><tt></tt></td><td></td><td><em>be</em></td></tr>
</table>

### `AUX`

4685 [en-pos/AUX]() tokens (48% of all `AUX` tokens) have a non-empty value of `Mood`.

The most frequent other feature values with which `AUX` and `Mood` co-occurred: <tt><a href="VerbForm.html">VerbForm</a>=Fin</tt> (4685; 100%), <tt><a href="Tense.html">Tense</a>=Pres</tt> (3406; 73%), <tt><a href="Person.html">Person</a>=EMPTY</tt> (2817; 60%), <tt><a href="Number.html">Number</a>=EMPTY</tt> (2814; 60%).

`AUX` tokens may have the following values of `Mood`:

* `Imp` (108; 2% of non-empty `Mood`): <em>do, be, get</em>
* `Ind` (4577; 98% of non-empty `Mood`): <em>have, do, is, was, are, has, did, were, had, does</em>
* `EMPTY` (5061): <em>will, can, would, be, could, should, been, may, 'll, have</em>

<table>
  <tr><th>Paradigm <i>be</i></th><th><tt>Ind</tt></th><th><tt>Imp</tt></th></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=1|<a href="Tense.html">Tense</a>=Past</tt></td><td><em>was</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=1|<a href="Tense.html">Tense</a>=Pres</tt></td><td><em>am</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=3|<a href="Tense.html">Tense</a>=Past</tt></td><td><em>was</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Person.html">Person</a>=3|<a href="Tense.html">Tense</a>=Pres</tt></td><td><em>is, 's, s</em></td><td></td></tr>
  <tr><td><tt><a href="Number.html">Number</a>=Sing|<a href="Tense.html">Tense</a>=Past</tt></td><td><em>was</em></td><td></td></tr>
  <tr><td><tt><a href="Tense.html">Tense</a>=Past</tt></td><td><em>were</em></td><td></td></tr>
  <tr><td><tt><a href="Tense.html">Tense</a>=Pres</tt></td><td><em>are, 'm, 're, r, m, re, ai</em></td><td></td></tr>
  <tr><td><tt></tt></td><td></td><td><em>be</em></td></tr>
</table>

### `ADP`

1 [en-pos/ADP]() tokens (0% of all `ADP` tokens) have a non-empty value of `Mood`.

`ADP` tokens may have the following values of `Mood`:

* `Ind` (1; 100% of non-empty `Mood`): <em>up</em>
* `EMPTY` (21679): <em>of, in, to, for, on, with, at, from, by, as</em>

## Relations with Agreement in `Mood`

The 10 most frequent relations where parent and child node agree in `Mood`:
<tt>VERB --[<a href="../dep/conj.html">conj</a>]--> VERB</tt> (1018; 60%),
<tt>VERB --[<a href="../dep/compound:prt.html">compound:prt</a>]--> VERB</tt> (1; 100%),
<tt>AUX --[<a href="../dep/aux.html">aux</a>]--> AUX</tt> (1; 100%),
<tt>AUX --[<a href="../dep/parataxis.html">parataxis</a>]--> VERB</tt> (1; 100%).

