...
6 <tt><a href="xav_xdt-pos-NOUN.html">NOUN</a></tt> tokens (1% of all `NOUN` tokens) have a non-empty value of `Emph`.

The most frequent other feature values with which `NOUN` and `Emph` co-occurred: <tt><a href="xav_xdt-feat-Number.html">Number</a></tt><tt>=Coll</tt> (5; 83%), <tt><a href="xav_xdt-feat-Person.html">Person</a></tt><tt>=EMPTY</tt> (4; 67%).

`NOUN` tokens may have the following values of `Emph`:

* `Yes` (6; 100% of non-empty `Emph`): <em>wahi'ratanorĩhã, ahã, wa'ranorĩhã, wamamanorĩhã</em>
