---
layout: base
title:  'Statistics of fixed in UD_Classical_Armenian-CAVaL'
udver: '2'
---

## Treebank Statistics: UD_Classical_Armenian-CAVaL: Relations: `fixed`

This relation is universal.

667 nodes (1%) are attached to their parents as `fixed`.

667 instances of `fixed` (100%) are left-to-right (parent precedes child).
Average distance between parent and child is 1.01499250374813.

The following 11 pairs of parts of speech are connected with `fixed`: <tt><a href="xcl_caval-pos-ADP.html">ADP</a></tt>-<tt><a href="xcl_caval-pos-ADV.html">ADV</a></tt> (398; 60% instances), <tt><a href="xcl_caval-pos-ADP.html">ADP</a></tt>-<tt><a href="xcl_caval-pos-ADP.html">ADP</a></tt> (188; 28% instances), <tt><a href="xcl_caval-pos-ADP.html">ADP</a></tt>-<tt><a href="xcl_caval-pos-NOUN.html">NOUN</a></tt> (44; 7% instances), <tt><a href="xcl_caval-pos-ADV.html">ADV</a></tt>-<tt><a href="xcl_caval-pos-SCONJ.html">SCONJ</a></tt> (15; 2% instances), <tt><a href="xcl_caval-pos-ADP.html">ADP</a></tt>-<tt><a href="xcl_caval-pos-SCONJ.html">SCONJ</a></tt> (10; 1% instances), <tt><a href="xcl_caval-pos-ADV.html">ADV</a></tt>-<tt><a href="xcl_caval-pos-ADV.html">ADV</a></tt> (4; 1% instances), <tt><a href="xcl_caval-pos-PART.html">PART</a></tt>-<tt><a href="xcl_caval-pos-PRON.html">PRON</a></tt> (3; 0% instances), <tt><a href="xcl_caval-pos-ADV.html">ADV</a></tt>-<tt><a href="xcl_caval-pos-PART.html">PART</a></tt> (2; 0% instances), <tt><a href="xcl_caval-pos-ADP.html">ADP</a></tt>-<tt><a href="xcl_caval-pos-ADJ.html">ADJ</a></tt> (1; 0% instances), <tt><a href="xcl_caval-pos-ADV.html">ADV</a></tt>-<tt><a href="xcl_caval-pos-ADP.html">ADP</a></tt> (1; 0% instances), <tt><a href="xcl_caval-pos-SCONJ.html">SCONJ</a></tt>-<tt><a href="xcl_caval-pos-SCONJ.html">SCONJ</a></tt> (1; 0% instances).


~~~ conllu
# visual-style 7	bgColor:blue
# visual-style 7	fgColor:white
# visual-style 6	bgColor:blue
# visual-style 6	fgColor:white
# visual-style 6 7 fixed	color:blue
1	ոչ	ոչ	PART	_	Polarity=Neg	2	advmod	_	Translit=očʻ|LTranslit=očʻ|Gloss=not
2	կարէ	կարել	VERB	_	Aspect=Imp|Mood=Ind|Number=Sing|Person=3|Tense=Pres|VerbForm=Fin|Voice=Act	0	root	_	Translit=karē|LTranslit=karel|Gloss=be_able
3	քաղաք	քաղաք	NOUN	_	Case=Nom|Number=Sing	2	nsubj	_	Translit=kʻałakʻ|LTranslit=kʻałakʻ|Gloss=city
4	թաքչել	թաքչել	VERB	_	VerbForm=Inf	2	xcomp	_	Translit=tʻakʻčʻel|LTranslit=tʻakʻčʻel|Gloss=hide
5	որ	որ	PRON	_	Case=Nom|Number=Sing|PronType=Rel	9	nsubj	_	Translit=or|LTranslit=or|Gloss=who
6	ի	ի	ADP	_	ExtPos=ADP	8	case	_	Translit=i|LTranslit=i|LId=ի-1|Gloss=to
7	վերայ	վերայ	ADV	_	_	6	fixed	_	Translit=veray|LTranslit=veray|Gloss=above
8	լերին	լեառն	NOUN	_	Case=Gen|Number=Sing	9	obl	_	Translit=lerin|LTranslit=leaṙn|Gloss=mountain
9	կայցէ	կալ	VERB	_	Aspect=Imp|Mood=Sub|Number=Sing|Person=3|VerbForm=Fin|Voice=Act	3	acl	_	SpaceAfter=No|Translit=kaycʻē|LTranslit=kal|Gloss=stand
10	՝	՝	PUNCT	_	_	2	punct	_	Translit=;|LTranslit=;

~~~


~~~ conllu
# visual-style 3	bgColor:blue
# visual-style 3	fgColor:white
# visual-style 2	bgColor:blue
# visual-style 2	fgColor:white
# visual-style 2 3 fixed	color:blue
1	եկին	գալ	VERB	_	Aspect=Perf|Mood=Ind|Number=Plur|Person=3|Tense=Past|VerbForm=Fin|Voice=Act	0	root	_	Translit=ekin|LTranslit=gal|Gloss=come
2	մինչեւ	մինչեւ	ADP	_	ExtPos=ADP	4	case	_	Translit=minčʻew|LTranslit=minčʻew|Gloss=until
3	առ	առ	ADP	_	_	2	fixed	_	Translit=aṙ|LTranslit=aṙ|Gloss=by
4	նա	նա	PRON	_	Case=Acc|Deixis=Remt|Number=Sing|PronType=Dem	1	obl	_	Translit=na|LTranslit=na|Gloss=he/she/it
5	եւ	եւ	CCONJ	_	_	6	cc	_	Translit=ew|LTranslit=ew|Gloss=and
6	արգելուին	արգելուլ	VERB	_	Aspect=Imp|Mood=Ind|Number=Plur|Person=3|Tense=Past|VerbForm=Fin	1	conj	_	Translit=argelowin|LTranslit=argelowl|Gloss=prohibit
7	զ	զ	ADP	_	Definite=Def	8	case	_	SpaceAfter=No|Translit=z|LTranslit=z
8	նա	նա	PRON	_	Case=Acc|Deixis=Remt|Number=Sing|PronType=Dem	6	obj	_	Translit=na|LTranslit=na|Gloss=he/she/it
9	զի	զի	SCONJ	_	_	11	mark	_	Translit=zi|LTranslit=zi|Gloss=because
10	մի	մի	PART	_	Polarity=Neg	11	advmod	_	Translit=mi|LTranslit=mi|Gloss=don't
11	գնասցէ	գնալ	VERB	_	Aspect=Perf|Mood=Sub|Number=Sing|Person=3|VerbForm=Fin|Voice=Act	6	advcl	_	Translit=gnascʻē|LTranslit=gnal|Gloss=go
12	ի	ի	ADP	_	_	13	case	_	Translit=i|LTranslit=i|LId=ի-3|Gloss=from
13	նոցանէ	նա	PRON	_	Case=Abl|Deixis=Remt|Number=Plur|PronType=Dem	11	obl	_	SpaceAfter=No|Translit=nocʻanē|LTranslit=na|Gloss=he/she/it
14	:	:	PUNCT	_	_	1	punct	_	Translit=.|LTranslit=.

~~~


~~~ conllu
# visual-style 12	bgColor:blue
# visual-style 12	fgColor:white
# visual-style 11	bgColor:blue
# visual-style 11	fgColor:white
# visual-style 11 12 fixed	color:blue
1	Հայր	հայր	NOUN	_	Case=Nom|Number=Sing	2	nsubj	_	Translit=hayr|LTranslit=hayr|Gloss=father
2	սիրէ	սիրել	VERB	_	Aspect=Imp|Mood=Ind|Number=Sing|Person=3|Tense=Pres|VerbForm=Fin|Voice=Act	0	root	_	Translit=sirē|LTranslit=sirel|Gloss=love
3	զ	զ	ADP	_	Definite=Def	4	case	_	SpaceAfter=No|Translit=z|LTranslit=z
4	որդի	որդի	NOUN	_	Case=Acc|Number=Sing	2	obj	_	SpaceAfter=No|Translit=ordi|LTranslit=ordi|Gloss=son
5	.	.	PUNCT	_	_	10	punct	_	Translit=:|LTranslit=:
6	եւ	եւ	CCONJ	_	_	10	cc	_	Translit=ew|LTranslit=ew|Gloss=and
7	զ	զ	ADP	_	Definite=Def	9	case	_	SpaceAfter=No|Translit=z|LTranslit=z
8	ամենայն	ամենայն	DET	_	Case=Acc|Number=Sing|PronType=Tot	9	det	_	Translit=amenayn|LTranslit=amenayn|Gloss=all
9	ինչ	ինչ	PRON	_	Animacy=Inan|Case=Acc|Definite=Ind|Number=Sing|PronType=Ind	10	obj	_	Translit=inčʻ|LTranslit=inčʻ|Gloss=something
10	ետ	տալ	VERB	_	Aspect=Perf|Mood=Ind|Number=Sing|Person=3|Tense=Past|VerbForm=Fin|Voice=Act	2	conj	_	Translit=et|LTranslit=tal|Gloss=give
11	ի	ի	ADP	_	ExtPos=ADP	13	case	_	Translit=i|LTranslit=i|LId=ի-1|Gloss=to
12	ձեռս	ձեռն	NOUN	_	Case=Acc|Number=Plur	11	fixed	_	Translit=jeṙs|LTranslit=jeṙn|Gloss=hand
13	նորա	նա	PRON	_	Case=Gen|Deixis=Remt|Number=Sing|PronType=Dem	10	nmod	_	SpaceAfter=No|Translit=nora|LTranslit=na|Gloss=he/she/it
14	:	:	PUNCT	_	_	2	punct	_	Translit=.|LTranslit=.

~~~


