---
layout: relation
title: 'advcl:purp'
shortdef: 'clause of purpose'
udver: '2'
---

A clause of purpose is a subordinate clause that expresses the purpose of an action. In Old Egyptian, a subjunctive verb form is used to express a consecutive clause.

~~~ conllu
# text = ꞽꞽ.n Wnꞽś ꞽ:Ꜥm ⸗f Ꜥnt.w
# trans = “Unas came to inhale myrrh (lit.: Unas came that he may swallow myrrh).”
1	ꞽꞽ.n	ꞽwi̯	VERB	_	Mood=Ind|SubForm=AbstRel|Tense=Past|VerbClass=Anom|VerbForm=Fin|Voice=Act	0	root	_	Hiero=𓇍𓈖|SPC=Past-2
2	Wnꞽś	Wnꞽś	PROPN	_	Gender=Masc	1	nsubj	_	Hiero=𓍹(𓃹:𓈖)𓇋𓋴𓍺|Name=King
3	ꞽ:Ꜥm	ꜥm	VERB	_	Mood=Sub|Prefix=Yodh|VerbClass=2lit|VerbForm=Fin	1	advcl:purp	_	Hiero=𓇋𓂝𓅓|SPC=Sub
4	⸗f	f	PRON	_	Gender=Masc|Number=Sing|Person=3|PronType=Prs	3	nsubj	_	Hiero=𓆑|Pron=SFP
5	Ꜥnt.w	Ꜥnt.w	NOUN	_	Gender=Masc|Number=Sing	3	obj	_	Hiero=(𓂝:𓈖)𓅂𓈒𓈒𓈒𓈒|Hierocl=𓈒𓈒𓈒𓈒

~~~

<!-- Interlanguage links updated Po 29. června 2026, 17:43:16 CEST -->
