---
layout: base
title:  'Statistics of PronType in UD_Indonesian-CSUI'
udver: '2'
---

## Treebank Statistics: UD_Indonesian-CSUI: Features: `PronType`
...
* `Dem` (109; 9% of non-empty `PronType`): <em>itu, demikian, ini, mana, begitu</em>
...
* `Tot` (42; 6% of non-empty `PronType`): <em>seluruh, semua, masing-masing, setiap, segala</em>

`PronType` seems to be **lexical feature** of `DET`. 100% lemmas (22) occur only with one value of `PronType`.

### `ADV`

1 <tt><a href="id_csui-pos-ADV.html">ADV</a></tt> tokens (0% of all `ADV` tokens) have a non-empty value of `PronType`.

`ADV` tokens may have the following values of `PronType`:

* `Rel` (1; 100% of non-empty `PronType`): <em>bagaimana</em>
* `EMPTY` (770): <em>lebih, juga, masih, sekitar, Sementara, tetap, kembali, sebelumnya, hanya, sangat</em>

### `NUM`

1 <tt><a href="id_csui-pos-NUM.html">NUM</a></tt> tokens (0% of all `NUM` tokens) have a non-empty value of `PronType`.

The most frequent other feature values with which `NUM` and `PronType` co-occurred: <tt><a href="id_csui-feat-NumType.html">NumType</a></tt><tt>=Card</tt> (1; 100%).

`NUM` tokens may have the following values of `PronType`:

* `Tot` (1; 100% of non-empty `PronType`): <em>Ke-23</em>
* `EMPTY` (1856): <em>2007, triliun, miliar, 2006, juta, 2008, satu, dua, 30, 10</em>
