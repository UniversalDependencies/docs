---
layout: base
title:  'Statistics of Foreign in UD_Old_East_Slavic-RNC'
udver: '2'
---

## Treebank Statistics: UD_Old_East_Slavic-RNC: Features: `Foreign`

This feature is universal.
It occurs with 1 different values: `Yes`.

1 tokens (0%) have a non-empty value of `Foreign`.
1 types (0%) occur at least once with a non-empty value of `Foreign`.
1 lemmas (0%) occur at least once with a non-empty value of `Foreign`.
The feature is used with 1 part-of-speech tags: <tt><a href="orv_rnc-pos-X.html">X</a></tt> (1; 0% instances).

### `X`

1 <tt><a href="orv_rnc-pos-X.html">X</a></tt> tokens (0% of all `X` tokens) have a non-empty value of `Foreign`.

`X` tokens may have the following values of `Foreign`:

* `Yes` (1; 100% of non-empty `Foreign`): <em>etc.</em>
* `EMPTY` (204): <em>л., [sic], </i>, <i>, {л._219}, {л._300}, {л._301_об.}, {л._302_об.}, {л._302}, 1</em>

