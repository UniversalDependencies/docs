...
### <a name="Yes">`Yes`</a>: emphatic form of a word

This value is marked either for words which appear in an expanded, emphatic form with respect to a basic one, and that can be no longer analyzed as a [compound](la-feat/Compound) if it ever was one, or for (functional) words (usually labelled as [particles](la-pos/PART)) which serve the purpose of creating an emphatic expression in association with another (lexical) word (usually taking the deprel [advmod:emph](la-dep/advmod-emph) like focus particles, which are mostly labelled as [adverbs](la-pos/ADV)).
