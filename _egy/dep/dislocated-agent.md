---
layout: relation
title: 'dislocated:agent'
shortdef: 'dislocated agent'
udver: '2'
---

In Old Egyptian, a dislocated agent is expressed by a postposed nominal agent that follows the preposition ꞽn in the man-impersonal construction. This dislocated agent is used as an epexegesis to determine the meaning of the noun tꞽ, which means "one" (lit. "body, person") and acts as the subject of the sentence. 

~~~ conllu
# text = ꞽḫ mk-t(ꞽ) Wnꞽś ꞽm ꞽn mꜣꜣ.w św
# trans = "So someone shall protect Unas there, namely those who see him."
1	ꞽḫ	ꞽḫ	PART	_	_	2	discourse	_	Hiero=𓇋𓐍
2-3	mk-t(ꞽ)	_	_	_	_	_	_	_	Hiero=𓅓(𓎡:𓏏)
2	mk	mki̯	VERB	_	Mood=Sub|VerbClass=3aeinf|VerbForm=Fin	0	root	_	Hiero=𓅓𓎡|SPC=Sub
3	t(ꞽ)	tꞽ	NOUN	_	Gender=Masc|Number=Sing	2	nsubj	_	Hiero=𓏏|Imprs=Man
4	Wnꞽś	Wnꞽś	PROPN	_	Gender=Masc	2	obj	_	Hiero=𓍹(𓃹:𓈖)𓇋𓋴𓍺|Name=King
5	ꞽm	ꞽm	ADV	_	AdvType=Loc	2	advmod	_	Hiero=𓇋𓅓
6	ꞽn	ꞽn	ADP	_	_	7	case	_	Hiero=𓇋𓈖|Status=Cons
7	mꜣꜣ.w	mꜣꜣ	VERB	_	Gender=Masc|Nominal=Yes|Number=Plur|Tense=Pres|VerbClass=2aered|VerbForm=Part|Voice=Act	2	dislocated:obl	_	Hiero=(𓌴:𓁹)𓄿𓄿𓅱
8	św	św	PRON	_	Gender=Masc|Number=Sing|Person=3|PronType=Prs	7	obj	_	Hiero=𓇓𓅱|Pron=Dep
~~~

<!-- Interlanguage links updated St 12. listopadu 2025, 09:21:03 CET -->
