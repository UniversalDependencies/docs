---
layout: base
title:  'Statistics of dep in UD_Amharic-ATT'
udver: '2'
---

## Treebank Statistics: UD_Amharic-ATT: Relations: `dep`

This relation is universal.

2 nodes (0%) are attached to their parents as `dep`.

2 instances of `dep` (100%) are left-to-right (parent precedes child).
Average distance between parent and child is 5.

The following 2 pairs of parts of speech are connected with `dep`: <tt><a href="am_att-pos-NOUN.html">NOUN</a></tt>-<tt><a href="am_att-pos-DET.html">DET</a></tt> (1; 50% instances), <tt><a href="am_att-pos-VERB.html">VERB</a></tt>-<tt><a href="am_att-pos-VERB.html">VERB</a></tt> (1; 50% instances).


~~~ conllu
# visual-style 9	bgColor:blue
# visual-style 9	fgColor:white
# visual-style 8	bgColor:blue
# visual-style 8	fgColor:white
# visual-style 8 9 dep	color:blue
1	ልጅ	ልጅ	NOUN	NOUN	_	15	nsubj	_	LTranslit=lədžə|Translit=lədžə
2	ኡ	ኡ	DET	DET	PronType=Art	1	det	_	LTranslit='u|Translit='u
3	ከ	ከ	ADP	ADP	_	4	case	_	LTranslit=kä|Translit=kä
4	መፍራት	መፍራት	NOUN	NOUN	VerbForm=Vnoun	7	xcomp	_	LTranslit=mäfəratə|Translit=mäfəratə
5	ኡ	ኡ	DET	DET	PronType=Art	4	det	_	LTranslit='u|Translit='u
6	የ	የ	PART	RLP	_	7	mark	_	LTranslit=jä|Translit=jä
7	ተነሣ	ተነሣ	VERB	VERB	Mood=Ind|VerbForm=Fin|Voice=Pass	15	advcl	_	LTranslit=tänäśa|Translit=tänäśa
8	መርፌ	መርፌ	NOUN	NOUN	_	13	obj	_	LTranslit=märəfe|Translit=märəfe
9	ው	ው	DET	DET	PronType=Art	8	dep	_	LTranslit=wə|Translit=wə
10	ን	ን	PART	ACC	_	8	case	_	LTranslit=nə|Translit=nə
11	ል	ል	SCONJ	SCONJ	_	13	mark	_	LTranslit=lə|Translit=lə
12	ይ	እሱ	PRON	SUBJC	Gender=Masc|Number=Sing|Person=3|PronType=Prs	13	nsubj	_	LTranslit='əsu|Translit=jə
13	ወጋ	ወጋ	VERB	VERB	Mood=Ind|VerbForm=Fin	15	xcomp	_	LTranslit=wäga|Translit=wäga
14	ኣል	ኣል	PART	NEG	Polarity=Neg	15	advmod	_	LTranslit='alə|Translit='alə
15	ደፈር	ደፈር	VERB	VERB	Mood=Ind|VerbForm=Fin	0	root	_	LTranslit=däfärə|Translit=däfärə
16	ኧ	እሱ	PRON	SUBJC	Gender=Masc|Number=Sing|Person=3|PronType=Prs	15	expl	_	LTranslit='əsu|Translit='ă
17	ም	ም	PART	NCM	_	15	discourse	_	LTranslit=mə|Translit=mə
18	።	።	PUNCT	PUNCT	_	15	punct	_	Translit=.|LTranslit=.

~~~


~~~ conllu
# visual-style 10	bgColor:blue
# visual-style 10	fgColor:white
# visual-style 1	bgColor:blue
# visual-style 1	fgColor:white
# visual-style 1 10 dep	color:blue
1	እባክ	እባክ	VERB	VERB	Mood=Ind|VerbForm=Fin	0	root	_	LTranslit='əbakə|Translit='əbakə
2	ህ	አንተ	PRON	SUBJC	Gender=Masc|Number=Sing|Person=2|PronType=Prs	1	nsubj	_	LTranslit='änətä|Translit=hə
3	ወንድም	ወንድም	NOUN	NOUN	_	1	obj	_	Translit=wänədəmə|LTranslit=wänədəmə
4	የ	የ	ADP	ADP	_	6	case	_	LTranslit=jä|Translit=jä
5	እዚህ	እዚህ	DET	DET	PronType=Dem	6	det	_	LTranslit='əzihə|Translit='əzihə
6	ቦታ	ቦታ	NOUN	NOUN	_	7	nmod	_	Translit=bota|LTranslit=bota
7	ስም	ስም	NOUN	NOUN	_	10	nsubj:pass	_	Translit=səmə|LTranslit=səmə
8	ማን	ማን	PRON	PRON	PronType=Int	10	obj	_	Translit=manə|LTranslit=manə
9	ይ	እሱ	PRON	SUBJC	Gender=Masc|Number=Sing|Person=3|PronType=Prs	10	expl	_	LTranslit='əsu|Translit=jə
10	ባል	ባል	VERB	VERB	Mood=Ind|VerbForm=Fin	1	dep	_	LTranslit=balə|Translit=balə
11	ኣል	ኣል	AUX	AUX	_	10	aux	_	LTranslit='alə|Translit='alə
12	?	?	PUNCT	PUNCT	_	1	punct	_	Translit=?|LTranslit=?

~~~


