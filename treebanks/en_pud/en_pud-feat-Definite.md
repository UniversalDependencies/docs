---
layout: base
title:  'Statistics of Definite in UD_English-PUD'
udver: '2'
---

## Treebank Statistics: UD_English-PUD: Features: `Definite`

This feature is universal.
It occurs with 2 different values: `Def`, `Ind`.

1885 tokens (9%) have a non-empty value of `Definite`.
6 types (0%) occur at least once with a non-empty value of `Definite`.
2 lemmas (0%) occur at least once with a non-empty value of `Definite`.
The feature is used with 1 part-of-speech tags: <tt><a href="en_pud-pos-DET.html">DET</a></tt> (1885; 9% instances).

### `DET`

1885 <tt><a href="en_pud-pos-DET.html">DET</a></tt> tokens (90% of all `DET` tokens) have a non-empty value of `Definite`.

The most frequent other feature values with which `DET` and `Definite` co-occurred: <tt><a href="en_pud-feat-PronType.html">PronType</a></tt><tt>=Art</tt> (1885; 100%).

`DET` tokens may have the following values of `Definite`:

* `Def` (1441; 76% of non-empty `Definite`): <em>the</em>
* `Ind` (444; 24% of non-empty `Definite`): <em>a, an</em>
* `EMPTY` (204): <em>this, all, that, some, these, each, no, both, another, every</em>

