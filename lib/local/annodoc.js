...
//         		      function(a, b) { return b.replace(/\s+/g, ' '); });
    ...
    // helper for *parse functions
    var logLine = function(logger, message, lineidx, line) {
        logger(message + ' (line '+(lineidx+1)+': "'+line+'")');
    }
    ...
    ConllUElement.prototype.validateId = function(id, issues) {
        ...
        if (!this.validateField(id, 'ID', issues)) {
            ...
            issues.push('ID must be integer or range: "'+this.id+'"');
            return false;
        } else {
            return true;
        }
    ...
    var removeIdDuplicates = function(elements, log) {
        // check for duplicate IDs, return de-dup'ed elements
        log = (log !== undefined ? log : makeLogger());
        ...
        for (var i=0; i<elements.length; i++) {
            ...
            if (elementById[element.id] === undefined) {
                ...
                logLine(log, 'error: non-unique ID "'+element.id+'"',
                        element.lineidx, element.line);
            }
        }
        return filtered;
    };

    var validateIdSequence = function(elements, log) {
        var error = false,
            expectedId = 1;
        ...
        for (var i=0; i<elements.length; i++) {
            ...
            if (parseInt(element.id, 10) !== expectedId) {
                logLine(log, 'error: word IDs should be 1,2,3,..., ' +
    ...
    var validateReferences = function(elements, log) {
        var elementById = {},
            error = false;

        for (var i=0; i<elements.length; i++) {
            elementById[elements[i].id] = elements[i];
        }

        for (var i=0; i<elements.length; i++) {
            var element = elements[i];

            // TODO: check that DEPREL is "root" iff HEAD is 0.

            // validate HEAD, repair if broken
            if (element.head !== '_' && element.head !== null && 
                element.head !== '0' && 
                elementById[element.head] === undefined) {
                logLine(log, 'error: HEAD is not valid ID: "'+element.head+'"',
                        element.lineidx, element.line);
                log('repair: blanking invalid HEAD');
                element.head = null;
                error = true;
            ...
            // validate DEPS, repair if broken
            if (element.deps === '_') {
                continue;
            }
            ...
            for (var j=0; j<deparr.length; j++) {
                ...
                if (m) {
                    ...
                    if (head !== '0' && elementById[head] !== undefined) {
                        ...
                        logLine(log, 'error: invalid ID "'+head+'" in DEPS',
                                element.lineidx, element.line);
                        log('repair: removing invalid ID from DEPS');
                        error = true;
                    }
                } else {
                    console.log('internal error: validateReferences(): ' +
                                'invalid DEPS');
                }                
            }
    ...
    var selectConllUSplitter = function(conll, log, strict) {
        ...
        if (conll.indexOf('\t') !== -1) {
            log('TAB found, parsing CoNLL-U in strict mode.')
            return strictConllUSplitter;
        } else {
            log('no TAB found, parsing CoNLL-U in loose mode.')
    ...
    var parseConllU = function(conll, log, strict) {
        ...
        for (var idx=0; idx<lines.length; idx++) {
            ...
            if (line.length !== 0 && line[0] === '#') {
                // comment line, check and skip
                if (!beforeSentence) {
                    log('error: comment lines in sentences disallowed');
                    error = true;
                }
                continue;
            }
            ...
            if (fields.length === 0) {
                ...
                if (elements.length === 0) {
                    logLine(log, 'error: empty sentence', idx, line);
                    error = true;
                } else {
                    sentences.push(elements);
                }
                // reset
                elements = [];
                beforeSentence = true;
            ...
            if (fields.length !== 10) {
                logLine(log, 'expected 10 fields, got '+fields.length, 
                        idx, line);              
                error = true;
            ...
            for (var j=0; j<issues.length; j++) {
                logLine(log, 'parse error: '+issues[j], idx, line);
            }
            if (issues.length !== 0) {
                error = true;
                // attempt repair
                if (!element.repair(log)) {
                    logLine(log, 'repair failed, discarding word', idx, line);
                    continue; // failed
                }
            }
        ...
        // TODO: the following logic only works with the last sentence,
        // generalize to operate over entire `sentences` array
        if (elements.length === 0 && sentences.length !== 0) {
            elements = sentences.slice(-1)[0];
        }

        // de-dup IDs
        var filtered = removeIdDuplicates(elements, log);
        ...
        // check ID sequences
        if (!validateIdSequence(elements, log)) {
            error = true;
        }

        if (!validateReferences(elements, log)) {
            error = true;
        }

        var dependencies = [];
        for (var i=0; i<elements.length; i++) {
            dependencies = dependencies.concat(elements[i].dependencies());
        }
        ...
        if (combinedText.length === 0 && error) {
            combinedText = '<FORMAT ERROR>';
        }
