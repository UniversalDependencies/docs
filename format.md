...
formatted as in the examples below. Moreover, if comments are used to specify the unannotated sentence as a single string, this comment should also be formatted as below. Comment and metadata lines inside sentences (i.e., between the token lines) are disallowed.
    ...
    5     clue      clue   NOUN    NN    Number=Sing                       2   oobj    _   SpaceAfter=No
