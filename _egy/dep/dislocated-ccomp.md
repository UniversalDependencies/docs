---
layout: relation
title: 'dislocated:ccomp'
shortdef: 'dislocated complement clause'
udver: '2'
---

In Old Egyptian, a dislocated complement clause is a clause that usually appears at the beginning of a sentence and is referred to by a pronominal object in the core of the sentence.

~~~ conllu
# text = ꜥnḫ.t ⸗f ꞽm ꜥnḫ Wnꞽś ꞽm
# trans = "What he (i.e. Horus) lives on, Unas lives on."
1	ꜥnḫ.t	ꜥnḫ	VERB	_	Gender=Fem|Mood=Ind|Nominal=Yes|Number=Sing|SubForm=RelForm|Tense=Pres|VerbClass=3lit|VerbForm=Fin	4	dislocated:ccomp	_	Hiero=𓋹𓏏|SPC=Pres
2	⸗f	f	PRON	_	Gender=Masc|Number=Sing|Person=3|PronType=Prs	1	nsubj	_	Hiero=𓆑|Pron=SFP
3	ꞽm	ꞽm	ADV	_	_	1	advmod	_	Hiero=𓇋𓅓
4	ꜥnḫ	ꜥnḫ	VERB	_	Mood=Ind|SubForm=AbstRel|Tense=Pres|VerbClass=3lit|VerbForm=Fin|Voice=Act	0	root	_	Hiero=𓋹|SPC=Pres
5	Wnꞽś	Wnꞽś	PROPN	_	Gender=Masc	4	nsubj	_	Hiero=𓍹(𓃹:𓈖)𓇋𓋴𓍺|Name=King
6	ꞽm	ꞽm	ADV	_	_	4	advmod	_	Hiero=𓇋𓅓

~~~


<!-- Interlanguage links updated Po 29. června 2026, 17:59:49 CEST -->
