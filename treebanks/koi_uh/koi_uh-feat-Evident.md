---
layout: base
title:  'Statistics of Evident in UD_Komi_Permyak-UH'
udver: '2'
---

## Treebank Statistics: UD_Komi_Permyak-UH: Features: `Evident`

This feature is universal but the values `Fh` are language-specific.
It occurs with 1 different values: `Fh`.

5 tokens (1%) have a non-empty value of `Evident`.
3 types (1%) occur at least once with a non-empty value of `Evident`.
2 lemmas (1%) occur at least once with a non-empty value of `Evident`.
The feature is used with 1 part-of-speech tags: <tt><a href="koi_uh-pos-VERB.html">VERB</a></tt> (5; 1% instances).

### `VERB`

5 <tt><a href="koi_uh-pos-VERB.html">VERB</a></tt> tokens (3% of all `VERB` tokens) have a non-empty value of `Evident`.

The most frequent other feature values with which `VERB` and `Evident` co-occurred: <tt><a href="koi_uh-feat-Mood.html">Mood</a></tt><tt>=Ind</tt> (5; 100%), <tt><a href="koi_uh-feat-Number.html">Number</a></tt><tt>=Sing</tt> (5; 100%), <tt><a href="koi_uh-feat-Person.html">Person</a></tt><tt>=3</tt> (5; 100%), <tt><a href="koi_uh-feat-Tense.html">Tense</a></tt><tt>=Past</tt> (5; 100%), <tt><a href="koi_uh-feat-Valency.html">Valency</a></tt><tt>=EMPTY</tt> (5; 100%), <tt><a href="koi_uh-feat-VerbForm.html">VerbForm</a></tt><tt>=EMPTY</tt> (5; 100%).

`VERB` tokens may have the following values of `Evident`:

* `Fh` (5; 100% of non-empty `Evident`): <em>краситӧм, краситӧма, чужӧма</em>
* `EMPTY` (142): <em>зэрӧ, босьтіс, быдмис, вӧтны, гижис, котӧртіс, осьт, петас, пондыліс, тшӧктіс</em>

