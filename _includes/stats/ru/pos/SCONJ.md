

--------------------------------------------------------------------------------

## Treebank Statistics (UD_Russian)

There are 1 `SCONJ` lemmas (7%), 26 `SCONJ` types (0%) and 624 `SCONJ` tokens (1%).
Out of 15 observed tags, the rank of `SCONJ` is: 12 in number of lemmas, 11 in number of types and 13 in number of tokens.

The 10 most frequent `SCONJ` lemmas: <em>_</em>

The 10 most frequent `SCONJ` types:  <em>что, где, когда, чтобы, как, так, хотя, если, поскольку, поэтому</em>

The 10 most frequent ambiguous lemmas: <em>_</em> ([NOUN]() 26660, [PUNCT]() 18807, [ADJ]() 12528, [ADP]() 10735, [VERB]() 9436, [PROPN]() 7604, [CONJ]() 3168, [ADV]() 2142, [NUM]() 1900, [PRON]() 1763, [X]() 1700, [DET]() 1673, [SCONJ]() 624, [PART]() 491, [SYM]() 158)

The 10 most frequent ambiguous types:  <em>что</em> ([SCONJ]() 250, [PRON]() 53, [DET]() 13, [ADP]() 12, [NOUN]() 1), <em>где</em> ([SCONJ]() 69, [ADV]() 25), <em>когда</em> ([SCONJ]() 46, [ADV]() 3), <em>как</em> ([ADP]() 175, [ADV]() 58, [SCONJ]() 30, [CONJ]() 4), <em>так</em> ([ADV]() 45, [SCONJ]() 27, [CONJ]() 8, [ADP]() 4), <em>хотя</em> ([SCONJ]() 21, [X]() 2, [CONJ]() 1), <em>если</em> ([SCONJ]() 15, [ADP]() 1), <em>поэтому</em> ([SCONJ]() 10, [ADV]() 1, [ADP]() 1), <em>пока</em> ([SCONJ]() 9, [ADV]() 5), <em>потому</em> ([ADV]() 9, [SCONJ]() 7, [ADP]() 3)


* <em>что</em>
  * [SCONJ]() 250: <em>Он утверждал , <b>что</b> по своей природе вирус жидкий .</em>
  * [PRON]() 53: <em><b>что</b> соответствует следующему разностному уравнению</em>
  * [DET]() 13: <em>Поэтому то , <b>что</b> произошло в следующем году , явилось самой настоящей сенсацией .</em>
  * [ADP]() 12: <em>Подходит человек и просит его пропустить , потому <b>что</b> он опаздывает на встречу .</em>
  * [NOUN]() 1: <em>Касательно I Am ... Sasha Fierce , отец , менеджер Бейонсе сказала : `` Мы думали нестандартно и придумали кое - <b>что</b> новое &#39;&#39; , а Бейонсе объяснила : `` Новая запись -- это двойной альбом , у него две обложки , как у журнала есть две обложки &#39;&#39; .</em>
* <em>где</em>
  * [SCONJ]() 69: <em>Самсоновка Ирбейского района был лагерь , совхоз , <b>где</b> содержались женщины .</em>
  * [ADV]() 25: <em>Участвовал в сражении при Головчине , <b>где</b> командовал левым флангом русской армии .</em>
* <em>когда</em>
  * [SCONJ]() 46: <em>А <b>когда</b> закончил , оказывается , я работал один &#39;&#39; .</em>
  * [ADV]() 3: <em>Евсевия вышла замуж за Констанция где - то в 352 -- 353 году , <b>когда</b> её отец уже был мертв .</em>
* <em>как</em>
  * [ADP]() 175: <em>Кассандра , <b>как</b> и сестра , замуж так и не вышла .</em>
  * [ADV]() 58: <em>Голова сверху не видна , так <b>как</b> прикрыта переднеспинкой .</em>
  * [SCONJ]() 30: <em>Мы не почувствовали , <b>как</b> вступили на землю Польши , а затем и Австрии .</em>
  * [CONJ]() 4: <em>В этом он близок <b>как</b> к традиции символизма так и к поэзии вещей ( Dinggedicht ) .</em>
* <em>так</em>
  * [ADV]() 45: <em>Кассандра , как и сестра , замуж <b>так</b> и не вышла .</em>
  * [SCONJ]() 27: <em>Голова сверху не видна , <b>так</b> как прикрыта переднеспинкой .</em>
  * [CONJ]() 8: <em>В этом он близок как к традиции символизма <b>так</b> и к поэзии вещей ( Dinggedicht ) .</em>
  * [ADP]() 4: <em>Они сочетают элементы византийского и современного мусульманского стиля , как в целом , <b>так</b> и в деталях .</em>
* <em>хотя</em>
  * [SCONJ]() 21: <em>Козы в качестве вожатых отброшены , <b>хотя</b> оракул ещё остается .</em>
  * [X]() 2: <em>`` Сегодня это не стоит в повестке дня и не надо это впихивать искусственно , не надо создавать наднациональных структур , которые не воспринимаются <b>хотя</b> бы одной страной &#39;&#39; .</em>
  * [CONJ]() 1: <em>Из - за забора воды на орошение и прочие хознужды , среднегодовая глубина реки в устье обычно не превышает 15 см , <b>хотя</b> в среднем течении оно доходит до 30 см , а верхнем , до 80 см ( по данным на август 2006 года ) .</em>
* <em>если</em>
  * [SCONJ]() 15: <em>Согласно Младшей Эдде , <b>если</b> Тор проголодался , он мог зажарить козлов себе на угощение .</em>
  * [ADP]() 1: <em>Такова логика всякой революции , тем более <b>если</b> она происходит где - нибудь на Востоке -- брат вынужден воевать против брата , отец -- против сына .</em>
* <em>поэтому</em>
  * [SCONJ]() 10: <em>Дождь является скорее исключением для Формулы - 1 , <b>поэтому</b> чаще всего используются шины для сухой трассы .</em>
  * [ADV]() 1: <em>Отчасти , вероятно , <b>поэтому</b> Лилиенкрон поступил в военное училище .</em>
  * [ADP]() 1: <em>В определённый момент Зепп узнаёт , что он популярен лишь потому , что является высокопоставленным чиновником , красавчиком и везунчиком , и <b>поэтому</b> начинает уважать и ценить преданность Кифа .</em>
* <em>пока</em>
  * [SCONJ]() 9: <em>Он избил её металлическим прутом , <b>пока</b> она не потеряла сознание .</em>
  * [ADV]() 5: <em>Со временем эту функцию возьмет на себя плацента , которой <b>пока</b> нет .</em>
* <em>потому</em>
  * [ADV]() 9: <em>Думаю , это одна из его лучших песен , <b>потому</b> что в ней отличные слова , -- я тут ни при чём .</em>
  * [SCONJ]() 7: <em>Подходит человек и просит его пропустить , <b>потому</b> что он опаздывает на встречу .</em>
  * [ADP]() 3: <em>Способ не слишком удачный : работает только для скриптов ( текстовых файлов на правах исполняемых ) , содержит абсолютный путь к программе и <b>потому</b> иногда этот путь приходится менять .</em>

## Morphology

The form / lemma ratio of `SCONJ` is 26.000000 (the average of all parts of speech is 2046.733333).

The 1st highest number of forms (26) was observed with the lemma “_”: <em>Where, Ибо, будто, ведь, где, дабы, если, как, когда, куда, откуда, пока, поскольку, потому, почему, поэтому, следовательно, словно, т., так, то, хоть, хотя, чем, что, чтобы</em>.

`SCONJ` occurs with 4 features: [ru-feat/Animacy]() (1; 0% instances), [ru-feat/Case]() (1; 0% instances), [ru-feat/Gender]() (1; 0% instances), [ru-feat/Number]() (1; 0% instances)

`SCONJ` occurs with 4 feature-value pairs: `Animacy=Inan`, `Case=Nom`, `Gender=Neut`, `Number=Sing`

`SCONJ` occurs with 2 feature combinations.
The most frequent feature combination is `_` (623 tokens).
Examples: <em>что, где, когда, чтобы, как, так, хотя, если, поскольку, поэтому</em>


## Relations

`SCONJ` nodes are attached to their parents using 1 different relations: [ru-dep/mark]() (624; 100% instances)

Parents of `SCONJ` nodes belong to 7 different parts of speech: [VERB]() (509; 82% instances), [NOUN]() (53; 8% instances), [ADJ]() (50; 8% instances), [ADV]() (7; 1% instances), [ADP]() (3; 0% instances), [PROPN]() (1; 0% instances), [PUNCT]() (1; 0% instances)

577 (92%) `SCONJ` nodes are leaves.

47 (8%) `SCONJ` nodes have one child.

The highest child degree of a `SCONJ` node is 1.

Children of `SCONJ` nodes are attached using 5 different relations: [ru-dep/mwe]() (40; 85% instances), [ru-dep/discourse]() (3; 6% instances), [ru-dep/punct]() (2; 4% instances), [ru-dep/advmod]() (1; 2% instances), [ru-dep/goeswith]() (1; 2% instances)

Children of `SCONJ` nodes belong to 4 different parts of speech: [ADV]() (30; 64% instances), [ADP]() (12; 26% instances), [X]() (3; 6% instances), [PUNCT]() (2; 4% instances)



--------------------------------------------------------------------------------

## Treebank Statistics (UD_Russian-SynTagRus)

There are 12 `SCONJ` lemmas (0%), 14 `SCONJ` types (0%) and 15498 `SCONJ` tokens (2%).
Out of 15 observed tags, the rank of `SCONJ` is: 13 in number of lemmas, 14 in number of types and 11 in number of tokens.

The 10 most frequent `SCONJ` lemmas: <em>ЧТО, КАК, ЕСЛИ, ЧТОБЫ, КОГДА, ХОТЯ, СЛОВНО, БУДТО, ЕДВА, ТОЧНО</em>

The 10 most frequent `SCONJ` types:  <em>что, как, если, чтобы, когда, хотя, словно, будто, чтоб, едва</em>

The 10 most frequent ambiguous lemmas: <em>ЧТО</em> ([SCONJ]() 6980, [PRON]() 2658, [PART]() 1), <em>КАК</em> ([SCONJ]() 3408, [ADV]() 1052), <em>КОГДА</em> ([SCONJ]() 1246, [ADV]() 217), <em>ХОТЯ</em> ([SCONJ]() 438, [PART]() 1), <em>СЛОВНО</em> ([SCONJ]() 71, [PART]() 26), <em>БУДТО</em> ([SCONJ]() 64, [PART]() 33), <em>ЕДВА</em> ([PART]() 24, [SCONJ]() 9), <em>ТОЧНО</em> ([ADV]() 193, [PART]() 10, [SCONJ]() 7), <em>СТОЛЬКО</em> ([ADV]() 136, [SCONJ]() 1), <em>ТАК</em> ([ADV]() 1869, [PART]() 11, [SCONJ]() 1)

The 10 most frequent ambiguous types:  <em>что</em> ([SCONJ]() 6946, [PRON]() 1527, [NOUN]() 1), <em>как</em> ([SCONJ]() 2890, [ADV]() 798), <em>когда</em> ([SCONJ]() 961, [ADV]() 200), <em>хотя</em> ([SCONJ]() 300, [PART]() 1), <em>словно</em> ([SCONJ]() 64, [PART]() 25), <em>будто</em> ([SCONJ]() 62, [PART]() 27), <em>едва</em> ([PART]() 21, [SCONJ]() 6), <em>точно</em> ([ADV]() 122, [PART]() 9, [SCONJ]() 7, [ADJ]() 6), <em>чем</em> ([CONJ]() 621, [PRON]() 316, [SCONJ]() 2), <em>столько</em> ([ADV]() 132, [SCONJ]() 1)


* <em>что</em>
  * [SCONJ]() 6946: <em>Донато установил , <b>что</b> они сделаны человеком .</em>
  * [PRON]() 1527: <em>" Всё , всё , всё ! " - радостно говорил он , обозначая то , <b>что</b> видел .</em>
  * [NOUN]() 1: <em>А толстяк этот - он <b>что</b> , тоже мертвый ?</em>
* <em>как</em>
  * [SCONJ]() 2890: <em>Их преодоление , <b>как</b> правило , связано с опасностью .</em>
  * [ADV]() 798: <em>Таким образом , наступало <b>как</b> бы равновесие .</em>
* <em>когда</em>
  * [SCONJ]() 961: <em>Но <b>когда</b> пятьдесят на пятьдесят , мы выбираем то , что хотим .</em>
  * [ADV]() 200: <em>Опыт подсказывает - открыться можно тогда , <b>когда</b> ты сам открываешься .</em>
* <em>хотя</em>
  * [SCONJ]() 300: <em>В школе Люда училась хорошо , <b>хотя</b> отличницей не была .</em>
  * [PART]() 1: <em>После отказа ( а это всегда был отказ , <b>хотя</b> и отложенный ) я дрожащими руками собирала свои вещи : мешочек с хлебом или картошкой , суковатую палку - клюку , сумочку со справками , заменявшими мне документы .</em>
* <em>словно</em>
  * [SCONJ]() 64: <em>Отрочества у Левы <b>словно</b> и не было - он учился в школе .</em>
  * [PART]() 25: <em>Он увидел платформу , с которой <b>словно</b> сдуло людей .</em>
* <em>будто</em>
  * [SCONJ]() 62: <em>Она мне казалась мешаниной , <b>будто</b> это украинский борщ .</em>
  * [PART]() 27: <em>Купил он <b>будто</b> хлеба - как обычно , батон круглый , бубликов десяток .</em>
* <em>едва</em>
  * [PART]() 21: <em>Мощности <b>едва</b> хватает на хлебозаводы и некоторые пищевые предприятия " .</em>
  * [SCONJ]() 6: <em>Он сразу поддержал Путина , <b>едва</b> президент окончательно утвердился в своем выборе .</em>
* <em>точно</em>
  * [ADV]() 122: <em>Он просто делал деньги , <b>точно</b> зная , что деньги в России - это власть .</em>
  * [PART]() 9: <em>Но <b>точно</b> не политиком .</em>
  * [SCONJ]() 7: <em>В руке у него был стакан водки , <b>точно</b> он хотел произнести тост .</em>
  * [ADJ]() 6: <em>Не мог он никого здесь знать - это уж <b>точно</b> .</em>
* <em>чем</em>
  * [CONJ]() 621: <em>" Как отец , но покрупнее , <b>чем</b> отец .</em>
  * [PRON]() 316: <em>В <b>чем</b> заключались " будущие вызовы " ?</em>
  * [SCONJ]() 2: <em>Такой подход оправдан : лучше вырастить одного уникального специалиста , открыть для него все двери , <b>чем</b> распылять усилия без уверенности в конечном успехе .</em>
* <em>столько</em>
  * [ADV]() 132: <em>Главное , что оснований для подозрений в обоих случаях <b>столько</b> же .</em>
  * [SCONJ]() 1: <em>По правде говоря , внешне оно совсем некрасиво , но его ценность определяется не <b>столько</b> формой , сколько содержанием .</em>

## Morphology

The form / lemma ratio of `SCONJ` is 1.166667 (the average of all parts of speech is 2.787274).

The 1st highest number of forms (2) was observed with the lemma “ЧТО”: <em>чем, что</em>.

The 2nd highest number of forms (2) was observed with the lemma “ЧТОБЫ”: <em>чтоб, чтобы</em>.

The 3rd highest number of forms (1) was observed with the lemma “БУДТО”: <em>будто</em>.

`SCONJ` does not occur with any features.


## Relations

`SCONJ` nodes are attached to their parents using 15 different relations: [ru-dep/mark]() (12668; 82% instances), [ru-dep/cc]() (2373; 15% instances), [ru-dep/parataxis]() (283; 2% instances), [ru-dep/mwe]() (89; 1% instances), [ru-dep/acl]() (24; 0% instances), [ru-dep/advmod]() (19; 0% instances), [ru-dep/name]() (11; 0% instances), [ru-dep/nmod]() (11; 0% instances), [ru-dep/root]() (5; 0% instances), [ru-dep/acl:relcl]() (3; 0% instances), [ru-dep/advcl]() (3; 0% instances), [ru-dep/conj]() (3; 0% instances), [ru-dep/nsubj]() (3; 0% instances), [ru-dep/dep]() (2; 0% instances), [ru-dep/appos]() (1; 0% instances)

Parents of `SCONJ` nodes belong to 11 different parts of speech: [VERB]() (11134; 72% instances), [NOUN]() (1769; 11% instances), [ADJ]() (1696; 11% instances), [ADV]() (505; 3% instances), [SCONJ]() (169; 1% instances), [PRON]() (126; 1% instances), [PART]() (65; 0% instances), [CONJ]() (13; 0% instances), [NUM]() (10; 0% instances), [SYM]() (6; 0% instances), [ROOT]() (5; 0% instances)

14196 (92%) `SCONJ` nodes are leaves.

1032 (7%) `SCONJ` nodes have one child.

180 (1%) `SCONJ` nodes have two children.

90 (1%) `SCONJ` nodes have three or more children.

The highest child degree of a `SCONJ` node is 9.

Children of `SCONJ` nodes are attached using 20 different relations: [ru-dep/punct]() (519; 30% instances), [ru-dep/dep]() (372; 22% instances), [ru-dep/advmod]() (326; 19% instances), [ru-dep/cc]() (226; 13% instances), [ru-dep/advcl]() (174; 10% instances), [ru-dep/nsubj]() (21; 1% instances), [ru-dep/parataxis]() (18; 1% instances), [ru-dep/cop]() (12; 1% instances), [ru-dep/mwe]() (12; 1% instances), [ru-dep/conj]() (9; 1% instances), [ru-dep/nmod]() (9; 1% instances), [ru-dep/neg]() (7; 0% instances), [ru-dep/mark]() (4; 0% instances), [ru-dep/appos]() (2; 0% instances), [ru-dep/acl]() (1; 0% instances), [ru-dep/amod]() (1; 0% instances), [ru-dep/aux]() (1; 0% instances), [ru-dep/dobj]() (1; 0% instances), [ru-dep/expl]() (1; 0% instances), [ru-dep/name]() (1; 0% instances)

Children of `SCONJ` nodes belong to 10 different parts of speech: [PUNCT]() (519; 30% instances), [NOUN]() (299; 17% instances), [PART]() (299; 17% instances), [SCONJ]() (169; 10% instances), [VERB]() (141; 8% instances), [ADJ]() (118; 7% instances), [ADV]() (90; 5% instances), [CONJ]() (62; 4% instances), [AUX]() (10; 1% instances), [PRON]() (10; 1% instances)

