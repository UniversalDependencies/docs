---
layout: base
title:  'Statistics of Voice in UD_Swedish-LinES'
udver: '2'
---

## Treebank Statistics: UD_Swedish-LinES: Features: `Voice`

This feature is universal.
It occurs with 2 different values: `Act`, `Pass`.

17101 tokens (17%) have a non-empty value of `Voice`.
3401 types (22%) occur at least once with a non-empty value of `Voice`.
1687 lemmas (16%) occur at least once with a non-empty value of `Voice`.
The feature is used with 3 part-of-speech tags: <tt><a href="sv_lines-pos-VERB.html">VERB</a></tt> (12200; 12% instances), <tt><a href="sv_lines-pos-AUX.html">AUX</a></tt> (4880; 5% instances), <tt><a href="sv_lines-pos-ADJ.html">ADJ</a></tt> (21; 0% instances).

### `VERB`

12200 <tt><a href="sv_lines-pos-VERB.html">VERB</a></tt> tokens (98% of all `VERB` tokens) have a non-empty value of `Voice`.

The most frequent other feature values with which `VERB` and `Voice` co-occurred: <tt><a href="sv_lines-feat-Mood.html">Mood</a></tt><tt>=Ind</tt> (7654; 63%), <tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Fin</tt> (7391; 61%).

`VERB` tokens may have the following values of `Voice`:

* `Act` (10988; 90% of non-empty `Voice`): <em>sa, hade, kom, såg, gick, har, ta, göra, se, tog</em>
* `Pass` (1212; 10% of non-empty `Voice`): <em>visas, kändes, används, tvungen, talas, hördes, tycktes, flyttas, skapats, döljs</em>
* `EMPTY` (215): <em>fanns, finns, vare, finnas, höll, åt, Avslöja, Hjälp, bestämma, förhörd</em>

<table>
  <tr><th>Paradigm <i>säga</i></th><th><tt>Act</tt></th><th><tt>Pass</tt></th></tr>
  <tr><td><tt><tt><a href="sv_lines-feat-Mood.html">Mood</a></tt><tt>=Imp</tt>|<tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Fin</tt></tt></td><td><em>säg</em></td><td></td></tr>
  <tr><td><tt><tt><a href="sv_lines-feat-Mood.html">Mood</a></tt><tt>=Ind</tt>|<tt><a href="sv_lines-feat-Tense.html">Tense</a></tt><tt>=Past</tt>|<tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Fin</tt></tt></td><td><em>sa, sade</em></td><td></td></tr>
  <tr><td><tt><tt><a href="sv_lines-feat-Mood.html">Mood</a></tt><tt>=Ind</tt>|<tt><a href="sv_lines-feat-Tense.html">Tense</a></tt><tt>=Pres</tt>|<tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Fin</tt></tt></td><td><em>säger</em></td><td><em>sägs</em></td></tr>
  <tr><td><tt><tt><a href="sv_lines-feat-Mood.html">Mood</a></tt><tt>=Ind</tt>|<tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Inf</tt></tt></td><td></td><td><em>sagt</em></td></tr>
  <tr><td><tt><tt><a href="sv_lines-feat-Mood.html">Mood</a></tt><tt>=Ind</tt>|<tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Part</tt></tt></td><td></td><td><em>sagt</em></td></tr>
  <tr><td><tt><tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Inf</tt></tt></td><td><em>säga</em></td><td></td></tr>
  <tr><td><tt><tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Sup</tt></tt></td><td><em>sagt</em></td><td><em>sagts</em></td></tr>
</table>

### `AUX`

4880 <tt><a href="sv_lines-pos-AUX.html">AUX</a></tt> tokens (100% of all `AUX` tokens) have a non-empty value of `Voice`.

The most frequent other feature values with which `AUX` and `Voice` co-occurred: <tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Fin</tt> (4346; 89%), <tt><a href="sv_lines-feat-Mood.html">Mood</a></tt><tt>=Ind</tt> (4318; 88%).

`AUX` tokens may have the following values of `Voice`:

* `Act` (4880; 100% of non-empty `Voice`): <em>var, är, hade, skulle, kan, har, kunde, vara, måste, varit</em>
* `EMPTY` (7): <em>var, haft, bli, skola</em>

`Voice` seems to be **lexical feature** of `AUX`. 100% lemmas (12) occur only with one value of `Voice`.

### `ADJ`

21 <tt><a href="sv_lines-pos-ADJ.html">ADJ</a></tt> tokens (0% of all `ADJ` tokens) have a non-empty value of `Voice`.

The most frequent other feature values with which `ADJ` and `Voice` co-occurred: <tt><a href="sv_lines-feat-Degree.html">Degree</a></tt><tt>=EMPTY</tt> (21; 100%), <tt><a href="sv_lines-feat-Tense.html">Tense</a></tt><tt>=EMPTY</tt> (20; 95%), <tt><a href="sv_lines-feat-VerbForm.html">VerbForm</a></tt><tt>=Part</tt> (20; 95%), <tt><a href="sv_lines-feat-Definite.html">Definite</a></tt><tt>=Ind</tt> (18; 86%), <tt><a href="sv_lines-feat-Case.html">Case</a></tt><tt>=Nom</tt> (12; 57%), <tt><a href="sv_lines-feat-Gender.html">Gender</a></tt><tt>=EMPTY</tt> (12; 57%), <tt><a href="sv_lines-feat-Number.html">Number</a></tt><tt>=Sing</tt> (12; 57%).

`ADJ` tokens may have the following values of `Voice`:

* `Pass` (21; 100% of non-empty `Voice`): <em>fastspända, fixerad, fixerade, fokuserad, född, förfärad, förtöjda, förödmjukade, gjorda, höjd</em>
* `EMPTY` (7134): <em>andra, själv, hela, samma, första, annat, många, enda, stor, flera</em>

`Voice` seems to be **lexical feature** of `ADJ`. 100% lemmas (20) occur only with one value of `Voice`.

## Relations with Agreement in `Voice`

The 10 most frequent relations where parent and child node agree in `Voice`:
<tt>VERB --[<tt><a href="sv_lines-dep-aux.html">aux</a></tt>]--> AUX</tt> (2184; 89%),
<tt>VERB --[<tt><a href="sv_lines-dep-conj.html">conj</a></tt>]--> VERB</tt> (1729; 90%),
<tt>VERB --[<tt><a href="sv_lines-dep-advcl.html">advcl</a></tt>]--> VERB</tt> (1020; 81%),
<tt>VERB --[<tt><a href="sv_lines-dep-xcomp.html">xcomp</a></tt>]--> VERB</tt> (673; 79%),
<tt>VERB --[<tt><a href="sv_lines-dep-ccomp.html">ccomp</a></tt>]--> VERB</tt> (651; 84%),
<tt>VERB --[<tt><a href="sv_lines-dep-parataxis.html">parataxis</a></tt>]--> VERB</tt> (259; 88%),
<tt>VERB --[<tt><a href="sv_lines-dep-csubj.html">csubj</a></tt>]--> VERB</tt> (61; 91%),
<tt>VERB --[<tt><a href="sv_lines-dep-advcl-relcl.html">advcl:relcl</a></tt>]--> VERB</tt> (17; 74%),
<tt>VERB --[<tt><a href="sv_lines-dep-ccomp.html">ccomp</a></tt>]--> AUX</tt> (17; 100%),
<tt>AUX --[<tt><a href="sv_lines-dep-aux.html">aux</a></tt>]--> AUX</tt> (13; 93%).

