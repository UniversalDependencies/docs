---
layout: base
title:  'Statistics of Foreign in UD_Indonesian-GSD'
udver: '2'
---

## Treebank Statistics: UD_Indonesian-GSD: Features: `Foreign`

This feature is universal.
It occurs with 1 different values: `Yes`.

89 tokens (0%) have a non-empty value of `Foreign`.
63 types (0%) occur at least once with a non-empty value of `Foreign`.
61 lemmas (0%) occur at least once with a non-empty value of `Foreign`.
The feature is used with 1 part-of-speech tags: <tt><a href="id_gsd-pos-X.html">X</a></tt> (89; 0% instances).

### `X`

89 <tt><a href="id_gsd-pos-X.html">X</a></tt> tokens (67% of all `X` tokens) have a non-empty value of `Foreign`.

`X` tokens may have the following values of `Foreign`:

* `Yes` (89; 100% of non-empty `Foreign`): <em>and, Image, in, kok, loh, x, assistent, b, resident, s</em>
* `EMPTY` (44): <em>I, a, akhiri, an, aplikasikan, baby, beratkan, beri, block, buang</em>

`Foreign` seems to be **lexical feature** of `X`. 100% lemmas (61) occur only with one value of `Foreign`.

## Relations with Agreement in `Foreign`

The 10 most frequent relations where parent and child node agree in `Foreign`:
<tt>X --[<tt><a href="id_gsd-dep-flat-foreign.html">flat:foreign</a></tt>]--> X</tt> (26; 93%),
<tt>X --[<tt><a href="id_gsd-dep-list.html">list</a></tt>]--> X</tt> (2; 100%),
<tt>X --[<tt><a href="id_gsd-dep-appos.html">appos</a></tt>]--> X</tt> (1; 100%),
<tt>X --[<tt><a href="id_gsd-dep-conj.html">conj</a></tt>]--> X</tt> (1; 100%).

