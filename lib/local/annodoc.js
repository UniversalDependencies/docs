...
//         		      function(a, b) { return b.replace(/\s+/g, ' '); });
    ...
    var ConllUSentence = function(elements) {
    ...
    ConllUSentence.prototype.toBrat = function(error) {
    ...
    ConllUSentence.prototype.addIssue = function(issue, element, issues) {
        issues.push(issue+'(line '+element.lineidx+1+': "'+ element.line+'")');
    }
    ...
    ConllUSentence.prototype.validate = function() {
        ...
        this.validateIdSequence(issues);
    ...
    ConllUSentence.prototype.validateUniqueIds = function(issues) {
        ...
        for (var i=0; i<this.elements.length; i++) {
            ...
            if (elementById[element.id] !== undefined) {
                this.addIssue('error: non-unique ID "'+element.id+'"',
                              element, issues);
            }
    ...
    ConllUSentence.prototype.validateIdSequence = function(issues) {
        issues = (issues !== undefined ? issues : []);
        ...
        for (var i=0; i<this.elements.length; i++) {
            ...
            if (parseInt(element.id, 10) !== expectedId) {
                this.addIssue('error: word IDs should be 1,2,3,..., ' +
    ...
    ConllUSentence.prototype.validateReferences = function(issues) {
        ...
        for (var i=0; i<this.elements.length; i++) {
            ...
            if (!element.validHeadReference(elementById)) {
                this.addIssue('error: HEAD is not valid ID: "'+element.head+'"',
                              element, issues);
            }
            ...
            for (var j=0; j<elemDeps.length; j++) {
                ...
                if (head !== '0' && elementById[head] === undefined) {
                    this.addIssue('error: invalid ID "'+head+'" in DEPS',
                                  element, issues);
                }
            }
        }
    ...
    ConllUSentence.prototype.repair = function(log) {
        ...
        if (!this.validateIdSequence()) {
            this.repairIdSequence(log);
        }
    ...
    ConllUSentence.prototype.repairIdSequence = function(log) {
        log('TODO: implement ConllUSentence.repairReferences()');
        return true;
    };
    ...
    var parseConllU = function(conll, log, strict) {
        ...
        for (var idx=0; idx<lines.length; idx++) {
            ...
            if (fields.length === 0) {
                ...
                if (elements.length === 0) {
                    ...
                    sentences.push(new ConllUSentence(elements));
                }
                // reset
                elements = [];
                beforeSentence = true;
        ...
        // expected terminating empty line. Process but warn.
        if (elements.length !== 0) {
            log('error: missing blank line after last sentence');
            if (strict) {
                error = true;
            }
            sentences.push(new ConllUSentence(elements));
        ...
        if (issues.length !== 0) {
            ...
            if (!sentence.repair(log)) {
                log('repair failed, discarding sentence');
                sentence.elements = []; // TODO: better resolution
            }
        }

        return sentence.toBrat(error);
    }
