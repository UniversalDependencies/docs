...
1407 tokens (1%) have a non-empty value of `NumForm`.
250 types (1%) occur at least once with a non-empty value of `NumForm`.
176 lemmas (1%) occur at least once with a non-empty value of `NumForm`.
The feature is used with 1 part-of-speech tags: <tt><a href="cs_fictree-pos-NUM.html">NUM</a></tt> (1407; 1% instances).

### `NUM`

1407 <tt><a href="cs_fictree-pos-NUM.html">NUM</a></tt> tokens (100% of all `NUM` tokens) have a non-empty value of `NumForm`.

The most frequent other feature values with which `NUM` and `NumForm` co-occurred: <tt><a href="cs_fictree-feat-NumType.html">NumType</a></tt><tt>=Card</tt> (1407; 100%), <tt><a href="cs_fictree-feat-Animacy.html">Animacy</a></tt><tt>=EMPTY</tt> (940; 67%), <tt><a href="cs_fictree-feat-NumValue.html">NumValue</a></tt><tt>=1,2,3</tt> (844; 60%), <tt><a href="cs_fictree-feat-Number.html">Number</a></tt><tt>=Sing</tt> (739; 53%).

`NUM` tokens may have the following values of `NumForm`:

* `Digit` (170; 12% of non-empty `NumForm`): <em>1, 11, 1942, 1948, 1953, 20, 10, 1945, 2, 7</em>
* `Word` (1237; 88% of non-empty `NumForm`): <em>pár, jeden, dva, dvě, jednoho, tři, jedna, jednu, oba, čtyři</em>
...
`NumForm` seems to be **lexical feature** of `NUM`. 99% lemmas (175) occur only with one value of `NumForm`.
