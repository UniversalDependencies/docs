---
layout: base
title:  'Statistics of Foreign in UD_Old_French-PROFITEROLE'
udver: '2'
---

## Treebank Statistics: UD_Old_French-PROFITEROLE: Features: `Foreign`

This feature is universal.
It occurs with 1 different values: `Yes`.

12 tokens (0%) have a non-empty value of `Foreign`.
8 types (0%) occur at least once with a non-empty value of `Foreign`.
2 lemmas (0%) occur at least once with a non-empty value of `Foreign`.
The feature is used with 4 part-of-speech tags: <tt><a href="fro_profiterole-pos-NOUN.html">NOUN</a></tt> (6; 0% instances), <tt><a href="fro_profiterole-pos-ADP.html">ADP</a></tt> (3; 0% instances), <tt><a href="fro_profiterole-pos-ADV.html">ADV</a></tt> (2; 0% instances), <tt><a href="fro_profiterole-pos-X.html">X</a></tt> (1; 0% instances).

### `NOUN`

6 <tt><a href="fro_profiterole-pos-NOUN.html">NOUN</a></tt> tokens (0% of all `NOUN` tokens) have a non-empty value of `Foreign`.

`NOUN` tokens may have the following values of `Foreign`:

* `Yes` (6; 100% of non-empty `Foreign`): <em>corpus, domini, damno, verbe</em>
* `EMPTY` (30170): <em>sire, rois, roi, cors, dame, gent, chevaliers, terre, seignor, reis</em>

### `ADP`

3 <tt><a href="fro_profiterole-pos-ADP.html">ADP</a></tt> tokens (0% of all `ADP` tokens) have a non-empty value of `Foreign`.

The most frequent other feature values with which `ADP` and `Foreign` co-occurred: <tt><a href="fro_profiterole-feat-Definite.html">Definite</a></tt><tt>=EMPTY</tt> (3; 100%), <tt><a href="fro_profiterole-feat-PronType.html">PronType</a></tt><tt>=EMPTY</tt> (3; 100%).

`ADP` tokens may have the following values of `Foreign`:

* `Yes` (3; 100% of non-empty `Foreign`): <em>in, en</em>
* `EMPTY` (19019): <em>de, a, en, par, por, d', au, des, del, as</em>

### `ADV`

2 <tt><a href="fro_profiterole-pos-ADV.html">ADV</a></tt> tokens (0% of all `ADV` tokens) have a non-empty value of `Foreign`.

The most frequent other feature values with which `ADV` and `Foreign` co-occurred: <tt><a href="fro_profiterole-feat-Polarity.html">Polarity</a></tt><tt>=EMPTY</tt> (2; 100%), <tt><a href="fro_profiterole-feat-PronType.html">PronType</a></tt><tt>=EMPTY</tt> (2; 100%).

`ADV` tokens may have the following values of `Foreign`:

* `Yes` (2; 100% of non-empty `Foreign`): <em>illo</em>
* `EMPTY` (23780): <em>si, ne, en, n', i, tant, bien, plus, ja, or</em>

### `X`

1 <tt><a href="fro_profiterole-pos-X.html">X</a></tt> tokens (14% of all `X` tokens) have a non-empty value of `Foreign`.

`X` tokens may have the following values of `Foreign`:

* `Yes` (1; 100% of non-empty `Foreign`): <em>Explycit</em>
* `EMPTY` (6): <em>qu'</em>

## Relations with Agreement in `Foreign`

The 10 most frequent relations where parent and child node agree in `Foreign`:
<tt>NOUN --[<tt><a href="fro_profiterole-dep-case.html">case</a></tt>]--> ADP</tt> (2; 67%),
<tt>NOUN --[<tt><a href="fro_profiterole-dep-nmod.html">nmod</a></tt>]--> NOUN</tt> (2; 100%).

