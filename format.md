...
orthographic words), it is sometimes necessary to extract multiple words out of a single white-space delimited tokens. 
...
We extract the word sequence by skipping all IDs that are not an integer:
