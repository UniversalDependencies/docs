...
Obl:arg [kitas šriftas] is also used to denote the instrumental case of nouns, adjectives, and participles with verbs: _laikyti (to consider), (pa)skelbti (to declare), (pa)skirti (to appoint), (pa)vadinti (to call), pripažinti (to recognize), virsti (to turn into), (pa)versti (to make, to turn into), įvardinti (to name)_:
~~~ sdparse
...
Prepositional constructions can be annotated either as [obl]() or `obl:arg`. The label [obl]() is used for prepositional constructions functioning adverbially (for example, Viską darau dėl tavęs – I do everything because of  you – adverbial of cause), whereas `obl:arg` is used to mark arguments traditionally referred to as indirect objects). It is also important to consider that `obl:arg` typically has restricted and [obl]() unrestricted valency. For example:
~~~ sdparse
