...
The head is the first part, the other parts are attached to it with the `goeswith` relation.
...
Note that we also use this relation to link the inflectional bound morphemes to the _“main”_ part of the word (usually phrases or sentences used as names) that are separated due to tokenization.

~~~ sdparse
« Երկիր/NOUN|Case=Nom Նաիրի/NOUN » - ից/X|Case=Abl \n “ Yerkir Nairi ” - from
punct(Նաիրի, «)
punct(Նաիրի, »)
nmod(Նաիրի, Երկիր)
punct(ից, -)
