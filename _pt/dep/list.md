...
The `list` relation is used for chains of comparable items. In lists
...
are labeled as `list`; the key-value pair relations are labeled
as [appos]().
...
Another place where `list` has been used is for a sequence of
...
However, `list` should not be over-used. If a construction can be
