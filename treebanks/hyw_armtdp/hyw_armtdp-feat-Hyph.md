---
layout: base
title:  'Statistics of Hyph in UD_Western_Armenian-ArmTDP'
udver: '2'
---

## Treebank Statistics: UD_Western_Armenian-ArmTDP: Features: `Hyph`

This feature is language-specific.
It occurs with 1 different values: `Yes`.

463 tokens (0%) have a non-empty value of `Hyph`.
33 types (0%) occur at least once with a non-empty value of `Hyph`.
13 lemmas (0%) occur at least once with a non-empty value of `Hyph`.
The feature is used with 3 part-of-speech tags: <tt><a href="hyw_armtdp-pos-NOUN.html">NOUN</a></tt> (443; 0% instances), <tt><a href="hyw_armtdp-pos-X.html">X</a></tt> (11; 0% instances), <tt><a href="hyw_armtdp-pos-ADJ.html">ADJ</a></tt> (9; 0% instances).

### `NOUN`

443 <tt><a href="hyw_armtdp-pos-NOUN.html">NOUN</a></tt> tokens (1% of all `NOUN` tokens) have a non-empty value of `Hyph`.

The most frequent other feature values with which `NOUN` and `Hyph` co-occurred: <tt><a href="hyw_armtdp-feat-Number.html">Number</a></tt><tt>=Sing</tt> (419; 95%), <tt><a href="hyw_armtdp-feat-Animacy.html">Animacy</a></tt><tt>=Nhum</tt> (413; 93%), <tt><a href="hyw_armtdp-feat-Case.html">Case</a></tt><tt>=Dat</tt> (289; 65%), <tt><a href="hyw_armtdp-feat-Definite.html">Definite</a></tt><tt>=Def</tt> (269; 61%).

`NOUN` tokens may have the following values of `Hyph`:

* `Yes` (443; 100% of non-empty `Hyph`): <em>ի, ին, ը, էն, ն, ները, ու, յի, ով, ս</em>

### `X`

11 <tt><a href="hyw_armtdp-pos-X.html">X</a></tt> tokens (2% of all `X` tokens) have a non-empty value of `Hyph`.

The most frequent other feature values with which `X` and `Hyph` co-occurred: <tt><a href="hyw_armtdp-feat-Foreign.html">Foreign</a></tt><tt>=EMPTY</tt> (11; 100%).

`X` tokens may have the following values of `Hyph`:

* `Yes` (11; 100% of non-empty `Hyph`): <em>հակա, փոխ</em>

### `ADJ`

9 <tt><a href="hyw_armtdp-pos-ADJ.html">ADJ</a></tt> tokens (0% of all `ADJ` tokens) have a non-empty value of `Hyph`.

The most frequent other feature values with which `ADJ` and `Hyph` co-occurred: <tt><a href="hyw_armtdp-feat-Degree.html">Degree</a></tt><tt>=EMPTY</tt> (9; 100%).

`ADJ` tokens may have the following values of `Hyph`:

* `Yes` (9; 100% of non-empty `Hyph`): <em>Խորհրդա, առասպելա, հոգինոց, պատճառա, պրոլետ, օրեայ, Շուէտ, իրանա, ռուսա</em>

