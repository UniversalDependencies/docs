...
This also includes the first element in paired conjunctions like _ni ... ni_ "nor ... nor".
...
**UD_French-Spoken** also uses the `cc` relation.
However the standard `conj` relation is not used anymore and has been replaced by a few subrelations (see [conj]() for more details).
...
In the following example, _c'est-à-dire_ is the coordinating conjunction in a [appos:conj]() relation:
