...
Strings that consist entirely of alphanumeric characters are not symbols but they may be [proper nouns](PROPN): _130XE_, _DC10_; other strings may be tagged `PROPN` (rather than `SYM`) even if they contain special characters: _ՏՈՒ-154Մ_ ("Tu-154M"). Similarly, abbreviations for single words are not symbols but are assigned the part of speech of the full form. For example, _պրն_ (պարոն "Mr.; Mister"), _կգ_ (կիլոգրամ "kg; kilogram"), _կմ_ (կիլոմետր "km; kilometer") should be tagged as `NOUN`. Acronyms for proper names such as _ՄԱԿ_ "UN" and _ՆԱՏՕ_ "NATO" should be tagged as `PROPN`.
