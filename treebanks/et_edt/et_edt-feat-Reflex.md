---
layout: base
title:  'Statistics of Reflex in UD_Estonian-EDT'
udver: '2'
---

## Treebank Statistics: UD_Estonian-EDT: Features: `Reflex`

This feature is universal.
It occurs with 1 different values: `Yes`.

1160 tokens (0%) have a non-empty value of `Reflex`.
68 types (0%) occur at least once with a non-empty value of `Reflex`.
7 lemmas (0%) occur at least once with a non-empty value of `Reflex`.
The feature is used with 1 part-of-speech tags: <tt><a href="et_edt-pos-PRON.html">PRON</a></tt> (1160; 0% instances).

### `PRON`

1160 <tt><a href="et_edt-pos-PRON.html">PRON</a></tt> tokens (5% of all `PRON` tokens) have a non-empty value of `Reflex`.

The most frequent other feature values with which `PRON` and `Reflex` co-occurred: <tt><a href="et_edt-feat-Person.html">Person</a></tt><tt>=EMPTY</tt> (1160; 100%), <tt><a href="et_edt-feat-PronType.html">PronType</a></tt><tt>=Prs</tt> (1099; 95%), <tt><a href="et_edt-feat-Number.html">Number</a></tt><tt>=Sing</tt> (1083; 93%).

`PRON` tokens may have the following values of `Reflex`:

* `Yes` (1160; 100% of non-empty `Reflex`): <em>ise, end, endale, enda, ennast, endast, endal, iseenda, enese, endaga</em>
* `EMPTY` (21857): <em>ta, see, oma, mis, ma, kes, seda, tema, mida, nad</em>

