...
In conjunction with the [case](case) relation, `nmod` provides a uniform analysis for the possessive alternation (with the option of a subtype like nmod:poss to distinguish non-adpositional case):

~~~ sdparse
սեղանի վրայի գիրքը \n the-book on of-the-table 
nmod(գիրքը, սեղանի)
case(սեղանի, վրայի)
nmod(the-book, of-the-table)
case(of-the-table, on)
~~~

նախագծի մասին որոշումը
