...
17 <tt><a href="lv_lvtb-pos-ADJ.html">ADJ</a></tt> tokens (0% of all `ADJ` tokens) have a non-empty value of `Typo`.

The most frequent other feature values with which `ADJ` and `Typo` co-occurred: <tt><a href="lv_lvtb-feat-NumType.html">NumType</a></tt><tt>=EMPTY</tt> (16; 94%), <tt><a href="lv_lvtb-feat-Degree.html">Degree</a></tt><tt>=Pos</tt> (14; 82%), <tt><a href="lv_lvtb-feat-Gender.html">Gender</a></tt><tt>=Fem</tt> (10; 59%), <tt><a href="lv_lvtb-feat-Number.html">Number</a></tt><tt>=Sing</tt> (10; 59%).
...
12 <tt><a href="lv_lvtb-pos-PROPN.html">PROPN</a></tt> tokens (0% of all `PROPN` tokens) have a non-empty value of `Typo`.

The most frequent other feature values with which `PROPN` and `Typo` co-occurred: <tt><a href="lv_lvtb-feat-Number.html">Number</a></tt><tt>=Sing</tt> (12; 100%).

`PROPN` tokens may have the following values of `Typo`:

* `Yes` (12; 100% of non-empty `Typo`): <em>Aigars, Andresena, Darjus, Eotekls, Grūtups, Latvija, OranžasU, Paura, Režijai, Rietumeiropa</em>

`Typo` seems to be **lexical feature** of `PROPN`. 100% lemmas (12) occur only with one value of `Typo`.

### `DET`

10 <tt><a href="lv_lvtb-pos-DET.html">DET</a></tt> tokens (0% of all `DET` tokens) have a non-empty value of `Typo`.

The most frequent other feature values with which `DET` and `Typo` co-occurred: <tt><a href="lv_lvtb-feat-Poss.html">Poss</a></tt><tt>=EMPTY</tt> (9; 90%), <tt><a href="lv_lvtb-feat-Gender.html">Gender</a></tt><tt>=Masc</tt> (8; 80%), <tt><a href="lv_lvtb-feat-Number.html">Number</a></tt><tt>=Sing</tt> (8; 80%), <tt><a href="lv_lvtb-feat-Person.html">Person</a></tt><tt>=EMPTY</tt> (8; 80%).

`DET` tokens may have the following values of `Typo`:

* `Yes` (10; 100% of non-empty `Typo`): <em>kurās, kāda, kādā, ne, savu, t, tas, visus, šadā, šāda</em>
...
6 <tt><a href="lv_lvtb-pos-ADP.html">ADP</a></tt> tokens (0% of all `ADP` tokens) have a non-empty value of `Typo`.

`ADP` tokens may have the following values of `Typo`:

* `Yes` (6; 100% of non-empty `Typo`): <em>par, ar, kar, pa, pat</em>

### `PART`

6 <tt><a href="lv_lvtb-pos-PART.html">PART</a></tt> tokens (0% of all `PART` tokens) have a non-empty value of `Typo`.

`PART` tokens may have the following values of `Typo`:

* `Yes` (6; 100% of non-empty `Typo`): <em>ari, var, jā</em>
